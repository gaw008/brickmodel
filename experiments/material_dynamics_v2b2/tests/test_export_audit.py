...
class ExportTests(unittest.TestCase):
    def test_raw_export_audit_and_mutations(self):
        ...
        failed=audit_exports(str(out.relative_to(ROOT)))
        self.assertFalse(failed['passed'])
        self.assertEqual(failed['fixed_tolerance'],1e-6)
        self.assertEqual(failed['failure']['scenario_id'],'C02')
        self.assertEqual(failed['failure']['tau'],0)
        self.assertEqual(failed['failure']['check'],'local_equimolar')
        self.assertAlmostEqual(failed['failure']['scaled_error'],.1)
