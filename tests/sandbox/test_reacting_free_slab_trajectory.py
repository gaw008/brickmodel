...
def test_reacting_free_trajectory_explicit_in_domain(water,tmp_path):
    ...
    try:
        ...
        for cap in (.00625,.003125):
            numerical=policy(initial_step_s=cap,maximum_step_s=cap,relative_tolerance=1e-9,maximum_wall_seconds=40.,
            ...
            for step,time,state in zip(out.steps,out.times_s[1:],out.states[1:],strict=True):
                expected=ref.sol(time)
                assert np.max(np.abs(state.internal_energy_j-expected[3:]))<2e-6
                assert np.max(np.abs(state.mechanical_stretches-expected[:3]))<2e-7
        ...
        assert runs[0].times_s!=runs[1].times_s
