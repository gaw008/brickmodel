...
def test_explicit_fixed_mode_matches_default_identity_and_values(water):
...
def test_fixed_host_explicitly_refuses_reacting_current_point(water):
    from test_free_solid_slab import host
    from sludge_sandbox.integration import IntegrationError
    base=host(water);points=[]
    for point in base.point_storages:
        skeleton=ManufacturedReactingSkeletonEnergy(reference_model=point.skeleton,composition_offset=1.,
            composition_weights_per_mol=(('fixture_solid',0.),),model_id='guard-reacting',version='1',
            classification='manufactured_test_fixture',allow_manufactured=True)
        points.append(replace(point,skeleton=skeleton,solid_inventory_regime='reacting_manufactured'))
    with pytest.raises(IntegrationError,match='reacting_current_points_not_admitted_by_fixed_free_slab'):
        replace(base,point_storages=tuple(points))
