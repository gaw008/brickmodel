...
@pytest.mark.parametrize('field', ['end', 'ordinary_policy', 'step_sizes'])
def test_reconstructed_original_request_cannot_change_under_rehash(packet, field):
    for path in sorted((packet / 'events').glob('*.json')):
        event = json.loads(path.read_bytes())
        if event['event'] == 'source_trajectory_reconstructed':
            graph = event['payload']
            graph['nodes'][graph['root']['ref']]['fields'][field] = 'altered-original-request'
            path.write_text(json.dumps(event))
            # The source metadata binds journal byte count, so preserve byte size
            # through trailing JSON whitespace after updating the expected count.
            mutate_observations(packet, lambda captures, meta: meta.update(
                journal_bytes=sum(len(p.read_bytes()) for p in (packet / 'events').glob('*.json'))))
            reseal_file(packet, 'events/' + path.name)
            break
    with pytest.raises(ValueError, match='original_reconstructed_problem'):
        read_source_trajectory_checkpoint(packet)


@pytest.mark.parametrize('failure', ['publication', 'deadline', 'late_cancel'])
def test_save_finalization_failure_retains_closed_session_and_unfinished_evidence(
        parent, tmp_path, monkeypatch, failure):
    import sludge_sandbox.source_trajectory_record as module
    session = open_test_session(parent, tmp_path / 'run', monkeypatch)
    assert session.advance(pause_after_steps=1).status == 'paused'
    real_json, real_time = module._json, module.time
    offset = [0.]
    module_time = SimpleNamespace(monotonic=lambda: real_time.monotonic() + offset[0],
                                  time_ns=real_time.time_ns)
    monkeypatch.setattr(module, 'time', module_time)
    def publish(path, value):
        result = real_json(path, value)
        if path.name == 'manifest.json':
            if failure == 'publication':
                raise OSError('manufactured publication failure after actual manifest')
            if failure == 'deadline':
                offset[0] = 2.
            else:
                session.recorder.cancel_callback = lambda: True
        return result
    monkeypatch.setattr(module, '_json', publish)
    directory = tmp_path / 'packet'
    with pytest.raises(Exception):
        save_source_trajectory(session, directory)
    assert session.closed and session.checkpoint is None
    assert (directory / 'FINALIZING.json').exists()
    assert not (directory / 'manifest.json').exists()
    saved = json.loads((directory / 'SAVE_FAILURE.json').read_bytes())
    assert saved['counts'] == session.recorder.counts and saved['elapsed_segment_seconds'] > 0
    if failure == 'late_cancel':
        assert saved['status'] == 'cancelled'
    with pytest.raises(ValueError, match='unfinished_finalization'):
        read_source_trajectory_checkpoint(directory)


def test_save_profile_guard_precedes_publication(tmp_path, monkeypatch):
    from sludge_sandbox.source_trajectory import SourceTrajectorySession
    # Explicit API guard projection only; no fake object is offered as physics.
    session = object.__new__(SourceTrajectorySession)
    session.built = SimpleNamespace(config=SimpleNamespace(values={'profile': 'old-profile'}))
    monkeypatch.setattr(SourceTrajectorySession, '_check', lambda self: None)
    with pytest.raises(ValueError, match='explicit_resume_profile'):
        save_source_trajectory(session, tmp_path / 'packet')
    assert not (tmp_path / 'packet').exists()


def test_raw_journal_scalar_types_and_shared_graph_work_are_preserved():
    from sludge_sandbox.source_trajectory_record import _raw_same, _raw_value
    from sludge_sandbox.source_run_journal import raw_projection
    assert not _raw_same({'shape': [3, 4]}, {'shape': [3., 4]})
    assert not _raw_same({'ordinal': 1}, {'ordinal': True})
    value = ('leaf',)
    for _ in range(22):
        value = (value, value)
    graph = raw_projection(value)
    left = _raw_value(graph, graph['root'])
    right = _raw_value(graph, graph['root'])
    assert left['values'][0] is left['values'][1]
    assert _raw_same(left, right)


def test_raw_cycle_and_explicit_work_limit_fail_closed(monkeypatch):
    import sludge_sandbox.source_trajectory_record as module
    graph = {'nodes': {'x': {'ref': 'x'}}}
    with pytest.raises(ValueError, match='raw_reference'):
        module._raw_value(graph, {'ref': 'x'})
    monkeypatch.setattr(module, 'MAX_RAW_VALUES', 4)
    with pytest.raises(ValueError, match='raw_work_limit'):
        module._raw_value({'nodes': {}}, list(range(10)))


def test_owned_managed_audit_keeps_shared_containers_and_rejects_cycles(monkeypatch):
    from types import MappingProxyType
    import sludge_sandbox.source_trajectory_record as module
    item = MappingProxyType({'status': 'verified'})
    for _ in range(22):
        item = (item, item)
    saved = MappingProxyType({'status': 'closed', 'primary_error': None,
        'secondary_errors': (), 'audit': MappingProxyType({'closed': True, 'rhs': item})})
    owned = module._owned_audit(saved)
    assert type(owned) is dict and type(owned['secondary_errors']) is list
    assert owned['audit']['rhs'][0] is owned['audit']['rhs'][1]
    cycle = []; cycle.append(cycle)
    with pytest.raises(ValueError, match='audit_cycle'):
        module._owned_audit(cycle)
    monkeypatch.setattr(module, 'MAX_RAW_VALUES', 4)
    with pytest.raises(ValueError, match='audit_work_limit'):
        module._owned_audit(list(range(10)))
