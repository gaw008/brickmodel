...
def test_multicell_sphere_robin_mode_spatial_and_temporal_convergence(water,tmp_path):
    ...
    evidence_path=tmp_path/'robin-grids.jsonl'
    def record(row):
        with evidence_path.open('a') as stream:stream.write(json.dumps(row)+'\n')
        print('ROBIN_GRID '+json.dumps(row),flush=True)
    for cells,dt in ((4,.001),(8,.001),(16,.001),(16,.0005)):
        ...
        # Measured first attempt hit 30s/resource cap. Only resource budget changed;
        # same physical case, grids, timesteps, Fo and numerical error gates.
        run=integrate(state,op,start_s=0.,end_s=end,
                      policy=replace(integration_policy(dt),maximum_wall_seconds=90.),
                      breakpoints_s=op.breakpoints_s(0.,end))
        resource={'cells':cells,'dt':dt,'status':run.status,'reason':run.reason,
                  'accepted':len(run.steps),'rejected':run.rejected_trials,'evaluations':run.evaluations,
                  'elapsed_s':time.monotonic()-started,'wall_budget_s':90.,
                  'last_time_s':run.times_s[-1],'required_end_s':end,'RMS_error_K':None}
        record(resource)
        ...
        errors.append(error)
        record({**resource,'RMS_error_K':error,'metric_status':'evaluated'})
