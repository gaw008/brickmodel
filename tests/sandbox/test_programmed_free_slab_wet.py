"""Actual water under a virtual furnace program and manufactured free slab."""
...
def snapshot(final):
    base=final.base_evaluation
    return {
        'boundary':encode(final.boundary), 'surface_temperature_k':final.surface_temperature_k,
        'heat':encode(final.heat), 'rates':encode(final.rates),
        'conductive_into_cell_w':final.conductive_into_cell_w,
        'surface_balance_residual_w':final.surface_balance_residual_w,
        'surface_balance_limit_w':final.surface_balance_limit_w,
        'surface_iterations':final.surface_iterations, 'surface_status':final.surface_status,
        'geometry':encode(base.geometry), 'free':encode(base.free),
        'storage_inverses':encode(base.storage_inverses), 'gas_states':encode(base.gas_states),
        'source_ids':encode(final.source_ids), 'operator_identity':encode(final.operator_identity),
...
def test_programmed_free_wet(ingredients,tmp_path):
    ...
    try:
        ...
        payload['initial']=encode(initial);payload['program']=encode(program)
        payload['operator_identity']=encode(wrapped.operator_identity)
        payload['source_ids']=encode(op.source_ids)
        payload['boundary_parameters']={name:encode(getattr(wrapped,name)) for name in
            ('convection_w_m2_k','emissivity','stefan_boltzmann_w_m2_k4','surface_policy',
             'coefficient_set_id','coefficient_version','coefficient_classification','coefficient_source_ids')}
        save();outputs=[];temps=[]
        for cap in (1e-4,2.5e-5):
            ...
            payload['runs'][-1]['final_callback']=snapshot(final);save()
