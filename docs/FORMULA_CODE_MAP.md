...
| `P_partial(t,x)=RT(t,x) sum_k[n_k,ref(t,x)/(phi_open(t,x) J(t,x))]` | `models.common.finalize_result`, `io.artifacts._forward_independent_semantics` | full pressure trajectory/extrema replay plus rehashed interior/extrema attacks |
...
| `Qreaction(t)=-rho_dry <sum_r DeltaH_r alpha_r(t)>`; `Qboundary(t)=rho_eff cp(<T(t)>-<T(0)>)-Qreaction(t)` | `models.l0/l1.rhs`, `io.artifacts._forward_independent_semantics` | every-time-point reaction/boundary cumulative replay and rehashed interior attacks; complete energy is `not_evaluated` |
...
| artifact SHA-256/size inventory + forward primary-state recomputation + inverse full-solver replay | `io.manifest`, `io.artifacts` | byte tamper; rehashed schema/unit/full-trajectory/summary/count/envelope/CSV/source/Pareto attacks; explicit integrity-only fields |
