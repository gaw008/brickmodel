...
Forward trajectory schema 2.0 plus semantic contract 3.0 retains every ODE/extensive state and gives every field exact species/unit/basis/conversion/shape/finite/range metadata. For normal synthetic L0/L1, manifest semantic contract 5.0 and `provenance.forward_semantic_replay` bind resolved case, fidelity, parameter/source hashes, exact parameter override digest, SciPy version and solver configuration digest. The deterministic baseline solver does not consume the forward seed, so the two self-declared seed copies provide only integrity consistency: missing or one-sided changes fail, while a coherent rewrite requires external authenticity and is not claimed as semantic tamper detection. Strict verification invokes `simulate` again in the verifier process, using the declared fidelity/configuration, and compares the replay time/spatial grids plus temperature, raw/projected extents, gas mass/molar/current-pore and released inventories, volume/Jacobian, porosity and cumulative heat primary states point-by-point.

Solver tolerances and semantic acceptance tolerances are separate. A verifier-owned, versioned range admits only finite numeric `solver_rtol` from `1e-10` through `1e-5` and `solver_atol` from `1e-13` through `1e-7`; all other values fail before replay. Those admitted values configure `solve_ivp` only. Recursive semantic comparisons canonicalize mapping keys and use independent verifier-owned hard caps of `rtol=5e-6` and `atol=5e-8`, which an artifact cannot widen.

After primary replay, replay-produced fields—not artifact-derived algebraic identities—are the expected values for ideal-gas pressure, liquid/sintering/stress trajectories, summary/extrema, conservation/status and CSV. Algebraic cross-artifact reconstruction remains defense in depth and endpoint reduced-enthalpy identity remains only an endpoint/numerical identity. Forward UQ payload, report prose, runtime/platform and solver work counters are integrity-only. An explicit custom/manufactured opt-out is labeled `forward_semantic_replay=not_evaluated`; its complete ODE trajectory is integrity-only and is not represented as semantically replayed.
