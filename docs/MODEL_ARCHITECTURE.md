...
Forward trajectory schema 2.0 plus semantic contract 3.0 retains every ODE/extensive state and gives every field exact species/unit/basis/conversion/shape/finite/range metadata. For normal synthetic L0/L1, manifest semantic contract 4.0 and `provenance.forward_semantic_replay` bind resolved case, fidelity, seed, parameter/source hashes, exact parameter override digest, SciPy version and solver configuration digest. Strict verification invokes `simulate` again in the verifier process, using the declared fidelity/configuration, and compares the replay time/spatial grids plus temperature, raw/projected extents, gas mass/molar/current-pore and released inventories, volume/Jacobian, porosity and cumulative heat primary states point-by-point. Recursive comparisons canonicalize mapping keys and use relative/absolute thresholds equal to five times the configured solve_ivp tolerances.
