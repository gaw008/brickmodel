...
用户授权持续开发到账户周额度剩余5%。[窗口记录](research/quota-window-20260924/REPORT.md)保留阶段范围与当前运行。新增自由水/吸附水条件模型的干燥及单格逆向加湿循环；原高W热量曲线对照与空间精度未通过，未给予材料资格。原W≤.8/低W通道另加入独立声明迁移率的凝聚相迁移，正在完成完整物理核查。128格物理反馈Jacobian两档轨迹运行中。原真实黏土M1的资料依赖未解除，没有替代Goal或完整烧砖成功声明。后续历史记录按各自时间理解。
