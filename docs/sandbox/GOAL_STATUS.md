...
当前实现/审核有界子任务已完成；主代理负责进度与安装证据。下一步设计审查由 review_water_resume 完成，只读意见已纳入下列步骤。Goal保持active，完整合同范围不变。

1. 将物种储能接到实际液水—气体机械闭合，实现沿P(T)路径的U→T解码并接有限体积核。固定p的MonotonicPath不能直接复用；需传播压力根、EOS和储能表示误差，不能只看最终U残差。
2. 已确认当前WaterState虽有Cp/Cv/u/h/rho，尚无公开αP、κT与∂u/∂p接口；RigidWaterGas只保存原始压力括区，尚未提供最终夹根区间。补齐这些误差传播所需接口，保留此前绑定源码/测试证据；不能把EOS残差或数值容差自动解释为严格物性误差界。
3. 固定库存闭合路径有 `C_closed≥ΣCv>0` 证明。可研究仅用活跃理想气全温区Cv下界提供保守数值下界，避免把未证明的用户常数当成自动证书；正性证明与已计算区间界须分开。
4. 实现有共同参考的相平衡/有限速率水迁移，继续原泥活度/毛细、输运、固相组成/参考能和独立实验来源；未知关系阻断材料预测。
5. 连续Shomate派生已可显式装入现有气热核，下一步仍需版本化序列化和IAPWS低温水汽衔接；随后推进烧结/闭孔/几何/冷却应力、实际多代搜索及CLI/UI。

当前平界面机械闭合、动态气热边界和连续气体热量路径已实际实现，勿重复停留输入层。沿机械路径的多相储能反演/相变和完整湿砖仍未实现；尚未满足合同第11节全部条件，不标记Goal完成。

后续实现的数值约定：新增公开液水导数α/κ并核查 `Cp−Cv=T v α²/κ`；局部压力敏感度为 `∂U_liq/∂P=P V_liq κ−T V_liq α`。点导数不当全区间界。压力残差转误差需包括EOS/体积求值误差，并可用 `B_min≥NgRT/P_high²`；当前尚无完整εF证明，不能直接拿既有pressure_resolution作界。能量反解应使用含物性/压力/表示误差的能量区间，模糊符号时细化或明确拒绝。复用PhaseStorage时各理想气传共同总压以获得可加偏摩尔体积，不能用分压重复累计全气孔体积。零液分支跳过水域，气体U与压力无关。验收需包含独立闭合导数、粗压力容差、生成能相消、零液高温、气体接缝和机械域中断。
