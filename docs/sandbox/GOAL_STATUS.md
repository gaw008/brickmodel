## 2026-09-27 06:19 UTC：迁移完成，存储目标仍未达

最后一条原refined128审计已退出0，最终JSON完成且原数值预算通过，耗时98,368.60s；材料资格仍为false。所有原计算均结束，不再轮询或重开。真实项目已迁至`~/Research/brickmodel-github`，Desktop仅留兼容链接；Git状态、分支及已有内容保持。最终审计17,165,929字节与三份收尾说明已补传私人Drive并核对元数据。

经171个保留gzip分片逐字节恢复比对，清理11,405,243,387字节原始重复轨迹。实际总占用162,907,435,008字节（项目及私有备份），20GB未达：本轮新增约29.75GB Git对象，观察到客户端后台差异查看触发按需拉取；准确匹配的读取/拉取进程已停止，Git对象与独有历史保留。云端完整下载恢复仍受既有403阻塞，不能据元数据核对删除唯一压缩原件。

本轮只更新迁移、备份、审计终态与存储证据，模型及验收没有重跑。后续仅处理可恢复性和实际存储目标，不扩展模型；以本节覆盖下方历史记录中“等待PID99759、真实目录仍在Desktop”的旧状态。详见[迁移报告](research/storage-migration-20260926/REPORT.md)。

## 2026-09-26 22:51 UTC：新增审计已补备份，迁移继续等待

base128最终审计10,976,520字节已补传私人Drive并核对云端名称、大小、父目录，本地原件保留。原refined128 PID99759启动身份、完整命令和Desktop cwd仍匹配且正在运行，未中断；迁移继续等待其退出。最新容量数据为22:12 UTC的144.52GB，20GB未达，完整云端下载恢复也未通过。私有迁移/存储记录已更新；详见[迁移报告](research/storage-migration-20260926/REPORT.md)。本次未更改模型或重跑验收。

## 2026-09-26 22:12 UTC：本轮模型扩展交付完成，后续仅迁移/存储

## 当前交付结论（2026-09-26）

VME现在可用根目录一个配置、一条命令连续运行湿坯→干燥→升温/脱羟/碳酸盐分解/有机物与炭氧化→烧结→冷却。有限孔气、扩散/Darcy流、压力反馈、石英热容/相变及有效轴向热弹性进入同一库存、能量与熵账本。175项主参数：38 literature、137 assumed、0 measured。材料适用性和真实砖预测仍待实测。

- 当前默认12格/60s、12格/30s和24格/60s均通过全程/分段质量、元素、能量及熵一致性；关键输出最大时间/空间差异约0.000901%/0.39468%，低于2%。原严格设置、库存分辨率说明和失败证据均保留。
- [新版UQ](../FULL_CYCLE_SOLID_UQ.json)：3方案×8组、24/24次通过，耗时882.81s（14.7分钟）；样本最坏阶段守恒相对残差0.02518%<0.1%。抽样9项，固定名义渗透率3.75e-14m²；广渗透率范围的两次成本受限探索并未完成，不隐瞒为通过。排名只在此条件与抽样子集下解释，8组样本尚不能证明要求的90%胜率，所有指标标为**稳健性未证实**。
- [新版合成标定](../FULL_CYCLE_SOLID_CALIBRATION.json)：TG、DSC、膨胀、窑温及吸水/强度/缺陷七类观测完成两参数拟合与实际文件导入。首次899.02s，工程设置下导入复核112.12s；恢复误差低于2%，参数状态保持assumed，未标为真实measured。
- 新运行无剩余fields.json原始场文件，Git只交付源码、配置及汇总/合成观测。历史数据仍约144.52GB（项目与私有备份合计），20GB未达。Desktop真实路径迁移仍等最后一条原refined审计PID99759，未中断；云端完整下载恢复仍未通过。

| 方案 | 终态孔隙率中位数 | 厚度收缩中位数 | 孔隙率5%—95%分位 |
|---|---:|---:|---:|
| 标准 | 0.47696 | 2.590% | 0.46854—0.48376 |
| 降低峰温 | 0.48161 | 1.717% | 0.47605—0.48607 |
| 延长保温 | 0.47056 | 3.767% | 0.45819—0.48057 |

分位区间是声明假设下的模拟分布，不是实验置信区间。此批次残碳均为数值零、峰值温差三个方案同分，不凭排序数组先后强分胜负。延长保温的孔隙率较低但收缩更大；不合成一个总分。强度、吸水和缺陷仍是未由实材标定的代理指标。

**近似边界**：固定面积一维半厚度、有效轴向准静力骨架、部分常数热物性、有限五反应总括网络；不含完整污泥热解产物谱、矿物液相演化、其他SiO2多晶型、石英转变体积跳变、真实裂纹或三维应力。既有结构内可替换参数并导入数据标定；引入这些缺失机制需要另外开发。本轮近似模型扩展的工程交付结束，不自动开展下一轮精度或功能扩展。迁移与历史存储继续按原授权收尾。

当前conditional-uq session77891已退出0，其他新标定/验收会话已关闭；不要重启。参数物理值与最终加密验收完全一致，只明确了UQ固定渗透率条件。后续只等待并处理原PID99759退出后的迁移，和满足可恢复性条件的历史存储清理；不得为耗尽额度或补未获新授权的机制继续扩展模型。

## 2026-09-26 21:56 UTC：UQ采用明确条件化范围

**本轮UQ适用范围调整**：十参数广范围探索的两轮运行已分别因计算成本有界停止（不是证明物理规律失败）。第二轮9次完整运行均通过，第10次在升温段耗时过长；同一组输入仅恢复渗透率为名义值后26.46s完整通过。两轮所有已完成结果、设置、未完成状态和实际退出130证据见FULL_CYCLE_FAILURES.json。按两次后简化的约定，不继续容差/网格搜索。本轮正式比较固定`transport.permeability_ref=3.75e-14 m²`，抽样其余9项；渗透率仍随孔隙率按原式演变，其模型方程和根参数允许范围没有删除或缩窄。这里的方案排名**以该名义渗透率为条件**，不能宣称已对渗透率范围或全部材料假设稳健。完整10参数探索仍未完成，是保留的适用限制。正在运行conditional-uq批次，旧批次均已关闭，不重启。 当前session77891，目录runs/full-cycle/conditional-uq，日志同名前缀。只剩此比较待完成；物理主机、三档加密和七类合成标定均已完成，数值设置不再改动。保护原PID99759，迁移/20GB仍待。

## 2026-09-26 21:34 UTC：工程容差验收与新版合成拟合通过

有限气体/石英/轴向骨架方程未变。高渗透率抽样慢运行已保留并仅结束新PID25582/session27325（退出130），旧refined PID99759未动。根ODE容差调整为1e-5/1e-7，重新三档验收通过；旧较紧设置、失败和配置均保留。首次新版合成拟合完成899.02s，物理一致性及2%参数恢复门槛通过、状态保持assumed。新完整UQ在runs/full-cycle/engineering-uq，session58085；当前容差导入拟合在engineering-calibration-import，session11144，均进行中，不重复启动。后续保存汇总、提交推送，达到本轮扩展验收即停止模型扩展；缺失机制如实记录。迁移仍待99759退出，唯一压缩原件与活动数据保护继续生效。

## 2026-09-26 21:14 UTC：石英与热弹性耦合验收完成

正式三档验收已经实际退出0：[FULL_CYCLE_SOLID_RESULTS.json](FULL_CYCLE_SOLID_RESULTS.json)。全程和每阶段质量/元素/能量最大相对残差0.00276659%；时间加密关键量最大变化0.000412537%，空间加密最大0.398467%，均低于2%。三档有效热容正、熵预算通过，机械平衡残差低于规定0.1%外压尺度。石英dH/dT=Cp=T dS/dT的采样相对差约4.14e-16。基准半砖终态密度1352.62kg/m³、孔隙率0.485625、厚度收缩0.95084%、峰值温差20.679K、超压418.30Pa；最高可逆热应变约0.925%。175项主参数：38 literature、137 assumed、0 measured。实现文件建立至本记录约8.7分钟；基准/时间/空间运行分别61.9/55.3/130.9秒。接下来在此版本运行三方案UQ和明确合成标定，旧版本结果不继承。 原base审计已退出0，仅refined PID99759仍阻塞迁移；20GB与云端完整恢复仍未通过。

## 2026-09-26 20:56 UTC：有限孔隙气体全流程验收完成

用户已再次授权继续补齐仿真机制，不能只继续旧存储收尾。VME已加入有限气体库存、真实孔压、摩尔扩散、Darcy流、孔压抵抗烧结与完整气体内能；共同热力学参考与共享面通量保持。完整12格/60s、12格/30s、24格/60s实际退出0，五项关键输出加密差异均低于2%，全程/每阶段守恒及熵通过。小于明确浮点分辨率的负产品数值保留、不裁剪；两次高成本方案和诊断记录保留。详见FULL_CYCLE_GAS_RESULTS.json及FULL_CYCLE_REPORT.md。157项主参数，25文献、132假设，无实测。当前继续石英热容/相变、热弹性与可逆膨胀耦合，再对升级版本运行三方案UQ和合成标定；旧结果不继承。原两条审计与路径保持不动，存储约151GB，迁移/20GB仍未完成。

## 2026-09-26：全流程近似开发四项完成，迁移与历史存储继续收尾

最新入口为[全流程报告](../FULL_CYCLE_REPORT.md)。用户再次要求继续开发后，保留旧进程和路径，在VME完成全流程、统一参数、三个方案UQ、七类合成观测拟合。12/24单元、60/30s的阶段守恒及2%加密门槛通过；材料与现实对照仍待实测。不得因下方历史旧目标重新启动模块精度扫描，也不得重跑已经完成的新验收。

18:46 UTC更新：迁移仍依赖两条原审计正常退出；两条full-access的256格续算已全部退出0，终态摘要保留，不追加旧路线审计。Git已精简至3.10GB，独有历史先归档并恢复核对，远端历史不改。本轮再清理16.67GB闲置重复文件，并将三条完整64格轨迹43.96GB无损归档为13.68GB后去掉原始重复文件；所有不完整状态/失败证据保留。首批8.573GB增量资料已上传且核对云端名称/大小/目录；完整64格归档及新结束的refined256待下一批补传。项目含私有备份约151.40GB，20GB尚未达到。云端下载403未解决，相关唯一原件保留。继续仅收尾迁移/数据恢复与存储，不重复已完成模型验收、Git归档或下载失败。

最新收尾：2026-09-26 19:45 UTC更新：第二批完整历史补备份已完成，1127个源文件分为44个包、260个云端分片，共21.218GB；另6个清单/恢复说明已上传。每个分片有大小及父目录读回，整批又按目标目录核对ID、名称与大小。两批补充合计362个数据分片、29.792GB及12个清单/说明文件，临时打包副本已清理。本地无损压缩源与独有Git历史继续保留，未将元数据核对写成远端下载恢复通过。
...
## 2026-09-26 11:47 UTC：等待迁移的旧任务降至两条

三条原任务已正常结束：开放64格精细积分、显式表面64格基础积分及方解石128格精细完整审计；另一个full-access开放64格基础完整审计也已通过原数值预算。两条积分的终态汇总、警告和两份审计结果保存，未将尚未执行的完整/时空审查记为通过。方解石既有空间五项失败保留。当前仅低温128格两条原审计仍阻塞迁移；两个full-access封闭256格恢复任务仍运行。遵守用户保留进度选择，未迁移、未重启或追加旧路线计算；新全流程合同继续按项目规范首节执行。

## 2026-09-26：用户调整为工程近似全流程交付

当前合同改为[项目规范首节](../BRICK_PHYSICS_PROJECT_SPEC.md)。sludge_vme为主机；按全流程、统一参数、不确定性、标定顺序推进。材料与现实对照改记“待实测”，不再阻塞串联；守恒0.1%、时间/空间各一次加密2%为工程验收。旧V0与旧阶段精度要求不再是当前开发门槛。迁移及旧计算结束方式仍按用户最新选择执行。尚未完成新全流程验收。

## 2026-09-26 09:00 UTC：方解石128格基础档完整审计通过

[方解石升降温记录](research/calcite-rigid-ramp-v1/REPORT.md)新增基础档完整审计结果：102943个接受步、14329216个保存格状态，来源状态、元素/能量/熵、表面与外界收支和2/4阶区间求积全部达到原预算。精细档完整审计仍在运行；既有64→128空间五项失败保留，不能据此称128格阶段或真实材料已获资格。material_qualified=false，training_eligible=false。原七条旧任务已正常结束两条，另外五条继续保留进度，目录迁移仍等待。

## 2026-09-26 07:44 UTC：64格开放边界基础档完成积分，独立完整审计已启动

[开放边界列记录](research/carbon-calcium-open-column-v1/REPORT.md)保存基础档30000s、54349接受步的实际终态；完整原始轨迹仍保留。已调用原有审计函数开始检查记录态、稠密区间及局部/整体元素、能量、熵和边界收支，原门槛不变，尚无完整审计通过结果。严格档、时间对照与32→64空间资格仍待；材料与训练资格保持false。原七条旧任务已有一条正常结束，其余六条继续保留进度，目录迁移仍等待。

## 2026-09-26 05:58 UTC：离线状态入口及存储迁移恢复

[研究状态入口](research/research-status-v1/REPORT.md)按原始结果路径读取七条主要研究线，区分失败和待产出。用户要求Drive备份并迁出Desktop；整体迁移暴露两个旧沙箱进程的路径写权限问题，已退回目录保护其余七条，用户选择待它们结束再迁移。[迁移及恢复记录](research/storage-migration-20260926/REPORT.md)保存两个256格不完整前缀、四项恢复点相同证据和显式BDF重新初始化；已从保存时刻连续推进，但完整积分/审计未完成。备份开始不等于全部上传完成，材料资格不变。

## 2026-09-26 05:14 UTC：高温缓存路径保持同值；偏高岭土参考焓补入来源算术

[高温精确物性缓存](research/carbon-calcium-thermal-cache-v1/REPORT.md)完成九个32格观察及两格30000s完整积分对照，11353接受步、6000样本和全部插值/物理字段相同；有限剖析仅约7%耗时差，未外推提速，也未重启长任务。[偏高岭土参考焓](research/metakaolin-reference-enthalpy-v1/REPORT.md)从公开水化量热文本重建两种Ca/Si下的报告值，元素及独立算术通过；配比纯度基准、原页目视、高温Cp/S等仍未知，未接入变温反应。另保留与Cantisani候选相差276kJ/mol的同参考态敏感性。

## 2026-09-26 05:00 UTC：组合热物性缓存完成完整数值等价核查

[方解石缓存](research/calcite-thermal-cache-v1/REPORT.md)组合单相与反应标准态的精确温度缓存，六个128格记录观察全部物理字段一致；四格20125步完整轨迹审计全部原预算通过。除设置元数据外，完整结果与原审计逐字段相同。后续审计可通过根参数显式启用；正在进行的大网格任务未重启，材料资格未改变。

## 2026-09-26 04:58 UTC：黏土煅烧来源的单位与适用域未闭合

[Cantisani来源与F3接口](research/cantisani-clay-source-v1/REPORT.md)完成2024原页及2025作者版追查，记录化学式、三阶浓度速率、体积单位和高岭石热容温区问题。18个热物性算术点及54组规范化F3摩尔速率独立核查通过；新接口仅支持明确公式单元池，未接入砖体能量模型，也未将缺陷来源修正后直接准入。Gariz上游技术报告未取得，2026全文正常获取返回403，材料依赖仍在。

04:12条目将“基础档二阶预算通过”归于低温128格，当前对应低温完整审计输出尚未出现该结果，不能沿用该通过说法。已实际见到二阶全部预算通过的是方解石128格基础档；低温两档完整审查继续待定。后续以各任务实际结果文件更新，不把另一模型的进度移用。本周额度实查剩余17%。

## 2026-09-26 04:39 UTC：有限速率温度关系未获预测资格

[Giammaria温度核查](research/giammaria2019-temperature-v1/REPORT.md)实现带温度和水分压导数的候选接口，42状态独立70位复算通过。原打印中心关系不能直接复现表2.1；在原能量范围内重新标定可覆盖全表，但逐次留出温点后干气仅2/4、水汽仅1/4落入源范围。候选保持独立，未接入砖体或变温能量轨迹，失败及来源区分已保存。现有大网格审查继续；额度剩余18%。

## 2026-09-26 04:23 UTC：方解石来源有限速率已连接受限热账

[Giammaria初始平台](research/giammaria2019-steam-plateau-v1/REPORT.md)实现固定老化试样的蒸汽依赖有限分解速率，明确表中系数含试样作用，不能按本征常数缩放。590°C标称值复现13个源点时11/13落入误差矩形，另两点失败保留；共同参数范围存在可行解，但使用同一拟合数据、未替换标称值。四个来源温度下52组声明的20s恒温有限库存/反应供热/出流熵数值核查通过。

这补入有来源的方解石有限正向速率及净反应热连接；CO₂反馈、逆反应、变温中间体储能、真实砖表面积/材料迁移与独立现实对照仍缺。高温和低温长轨迹完整审查继续。

## 2026-09-26 04:12 UTC：新增真实碳酸化循环数据

[Donat2025原始表格核查](research/donat2025-carbonation-data-v1/REPORT.md)导入十条归一化TGA循环、16000点，逐字段CSV回读相同。第1→5循环干气终点吸收量下降36.74%，湿气上升7.17%；47个负基线点保留。固定文中近似容量后的区间一阶速率并非常数，尚未拟合或认定本征动力学。长期实验650°C图注与700°C工作簿冲突保留，不连接能量模型。

低温128格5个研究附件及3份清单现已实际上传并逐字节远端回读一致；基础档二阶完整收支预算通过、四阶继续，两档完整资格尚未结束。高温大网格继续；本周额度余额19%。

## 2026-09-26 03:55 UTC：脱羟后热储来源差异

[热容版本核查](research/michot-caloric-source-review-v1/REPORT.md)取得Michot2008博士论文并目视核对原页，记录2011期刊摘要的另一公式。2008打印式在1000°C不落入同页图的宽读取包络；两版1100°C热容差18.66%，40→1100°C显热差82.17kJ/kg。新增固定状态Cp及相对H/S计算，独立求积通过，但不供应生成焓、绝对熵或反应热，未接入F1/F3或砖体能量方程。低温128格两条轨迹已完成无损压缩和五个附件包的逐成员回读；尚未上传，完整物理审查仍待结果。

## 2026-09-26 03:45 UTC：后续大轨迹保存方式调整

GitHub成功接收f1181dc5，但提示仓库接近容量上限；API当前报告86669181 KiB。后续大轨迹准备使用同项目研究预发布附件，代码、参数、来源、报告和清单继续Git提交，已有历史保持。低温表面128格无损压缩进行中，附件尚未创建或完成下载回读，原始轨迹未删除。完整物理核查仍在运行，存储完成不等于数值或材料通过。

## 2026-09-26 03:37 UTC 增量：现代高岭石实验的条件迁移对照

[高岭石现代实验对照](research/kaolinite-modern-validation-v1/REPORT.md)取得2020/2025公开原文，提取2025两种材料五个升温速度的89个可读转化率点，保留一个被遮挡点。固定3°C/min图上参数、每条曲线提供α=0.1初态后，79个后续点的F3数值实现通过独立60位求积及两档精度；SRB高速预测最大α偏差0.12853，6点仍超包住图中隐藏误差棒的宽范围，不能称任意温程已验证。原始仪器数据、完整误差、偏高岭土热储及同材料砖体验证仍缺，未接入能量方程。其余大网格完整审查继续。

## 2026-09-26 03:17 UTC 增量：黏土热化学与限定脱羟速率

[高岭石来源核查](research/kaolinite-source-review-v1/REPORT.md)完成四相28个温点、84个原表字段及两条结晶产物反应的独立热化学核对，共同温区止于800K。原书高岭石分子式疑似误印及石英两字段末位差异显式保留；计入打印系数范围后表算术覆盖，不冒充实验误差。六个限定反应自由能界限完成独立求根，但不能当作真实脱羟起始温度。

Vaughan1955的13行速率参数完成追查：地开石半反应时间存在无法由打印范围解释的矛盾，保留失败。新增CornwallChinaClay2的指定温度一阶脱羟示例，独立积分及两档时间通过；原表半反应时间并非独立测量，变温使用、偏高岭土热储和真实砖体耦合未获资格。其他128格完整审查及高温256格继续，材料和全周期缺项未解除。

## 2026-09-26 02:50 UTC 增量：有限气流与变形孔隙接通；128格资格分项更新

[可变球孔—有限气室](research/effusive-spherical-pore-v1/REPORT.md)接通N₂出入、两侧温度、库存热储、球孔收缩功和熵。36静态状态及收紧精度的8条30s轨迹完整核查通过；原7条局部库存积分失败及4组时间失败保留，未改预算。这是无反应、虚拟基体和理想分子流口的条件研究模型，尚无实际砖孔网络或材料资格。

[高温封闭128格](research/carbon-calcium-temperature-column-v1/REPORT.md)完整来源/逐格守恒/熵与时间通过；64→128平均T、父格T、平均P通过，物种总量差2.1003e−7mol仍超1e−8mol。已完成[无损分片直接保存](research/partitioned-trajectory-storage-v1/REPORT.md)的原文及全部字段回读，开始256格两档，资格待定。[低温表面128格](research/sorptive-surface-tight-128-v1/REPORT.md)两档时间通过，完整审查仍在运行。方解石128格两档時間比较通过，64→128温度、压力、相量和表面温度仍失败，完整审查尚未结束。

02:46 UTC按M1通讯作者地址只读查询仍无新来信；未发送追问。真实黏土干燥、同配方湿到高温热储、有限反应速率、结构/冷却和完整留出对照缺项不变。material_qualified=false，training_eligible=false。

## 2026-09-26 02:20 UTC 增量：公开烧结记录对照与原始理论追查

[Wadsworth2016 玻璃烧结对照](research/wadsworth2016-sintering-data-v1/REPORT.md)完成 12 组、2283 行公开记录导入，保留原始温度历程及异常观测。固定公开孔径、孔隙率、表面张力及显式解释的 VFT 黏度，独立高精度时间积分、球孔动力学、时间加密及共享机械关系核查均通过；有定义的 1242 点归一化孔隙率平均绝对偏差约 0.06579，另 1041 点已越过该开孔模型的有限闭合时间，不能宣称完整真实烧结预测通过。两条负孔隙率及对应负 Error 原样保存，没有修剪或取绝对值。

[2017 后续来源与 1992 原始理论](research/wadsworth2017-source-review-v1/REPORT.md)确认上述 VFT 的 log10 形式，同时发现2017打印多粒径式的概率积分不为1；三个单粒径极限算例均失败。1992原始表面距离公式可解释该极限的变量错位，但没有正式勘误或一般多粒径材料准入。玻璃数据没有替代已选 M1 黏土砖坯目标。其余长轨迹的完整审查继续，material_qualified=false，training_eligible=false。
...
## 2026-09-25 17:05 UTC 增量：表面八格完整资格，导数性能失败保留

[显式表面八格](research/carbon-calcium-surface-column-v1/REPORT.md)全来源、表面与逐格/外界收支、熵、时间资格通过，4→8空间四项仍失败。[Jacobian改进](research/carbon-calcium-open-colored-jacobian-v1/REPORT.md)保留四个固定差分相变附近低效前缀，以及小步长总熵求差参考的相消失败；80位全部面差分与自适应体分组静态比较通过，完整两格资格仍在运行。旧方解石开放64更紧档已完成79511步，完整审查启动；直接接触16已完成，完整审查进行中。实砖/有限动力学/全周期阻塞不变。

## 2026-09-25 16:48 UTC 增量：氮氧气零密度文献关联式复现

[Lemmon2004零密度η/λ及导数](research/lemmon2004-dilute-transport-v1/REPORT.md)96字段80位复算与原文四状态八个计算值都达到冻结预算。O2黏度与旧NIST表8/8相符，但O2导热率6/8超出旧表范围，最大10.23%，N2导热率1/8超出；不能把数值复现称全部物性已验证。来源版本和压力条件差异保留待核。显式表面八格积分完成，完整收支核查启动；直接接触十六格及其余长轨迹继续。

## 2026-09-25 16:38 UTC 增量：开放八格/表面四格资格，气体物性偏差保留

直接接触八格与显式表面四格两档均完成全部来源、局部及外界收支、熵和时间资格，空间四项仍失败；各自十六格/八格按原预算继续。[四气体来源输运](research/gas-transport-source-v1/REPORT.md)的40组黏度、180组二元扩散算术与来源核对通过，但氧气黏度8个历史表点中7个超出原参考范围，最大2.50%偏差，保留失败。未将自由气体D直接替换虚拟孔内L，未解除真实材料或全周期阻塞。

## 2026-09-25 16:32 UTC 增量：表面连接两格时间资格，空间仍失败

[显式表面列](research/carbon-calcium-surface-column-v1/REPORT.md)两格两档全部来源、局部/外界收支、表面余额、熵与时间预算通过；1→2空间四项失败，最大均温差15.708K。四格已经完成积分，完整核查与空间比较推进中。气体输运来源补充正在核对NIST公开表与固定版本Cantera来源，尚未替换虚拟多孔介质系数。实砖与全过程资格保持未通过。

## 2026-09-25 16:15 UTC 增量：表面连接列单格全过程资格

[显式零储量表面列](research/carbon-calcium-surface-column-v1/REPORT.md)18组非均匀变化率及单格两档30000s全来源/逐格与外界收支/表面闭合/熵/2-4求积/时间比较全部通过。表面与内部热流分开保存，辐射只在表面输入；记录最大表面—体温差45.477K不代表实砖测量。下一两格阶段和原直接接触八格分别推进，各自保留空间未资格结论。真实材料、有限速率与全周期缺项不变。

## 2026-09-25 16:10 UTC 增量：开放四格资格与独立表面关系

[开放共同反应列](research/carbon-calcium-open-column-v1/REPORT.md)两格、四格都已完成全来源/局部与外界收支/熵/时间资格，2→4空间四项仍失败。[零储量表面](research/carbon-calcium-integral-surface-v1/REPORT.md)21组独立80位五未知量平衡、三个串联/零流极限和指定条件下根唯一性通过；已有模型温区与原文温区不同的首次失败保留，未扩展物性范围。表面连接列18组非均匀来源与变化率核对通过，单格完整积分审查刚开始。封闭64积分完成，完整审查进行中；低温128和开放64更紧档继续。真实材料、有限速率和全周期资格保持未通过。

## 2026-09-25 15:42 UTC 增量：开放共同反应列单格资格

[一维开放Ca/C/O共同反应装配](research/carbon-calcium-open-column-v1/REPORT.md)的1/2/4格18组来源与局部元素/U/S率核查通过；单格两档30000s全部局部/外界/熵/2-4求积及时间预算通过，并与原单腔6000时刻对照通过。所有完整核查节点处于来源温度范围内。两格已开始，末控制体直接接外界的离散近似和空间误差待独立核查；真实砖资格不变。

## 2026-09-25 15:29 UTC 增量：独立气体/辐射连续程序资格

[共同反应连续热程序](research/carbon-calcium-radiative-program-v1/REPORT.md)完成两档30000s加热/保持/冷却/保持，全部来源、元素/U、分热源能量与熵、2/4求积和时间预算通过。记录到热流反向及富CO2冷却再碳酸化。末态低温粗差分失败保留，更小来源差分与生产热学响应逐字段核对通过；物理参数和门槛未改。前序方解石开放64精细完整审查通过，更紧档正在补齐时间对照；共同反应64和低温128的独立数值任务仍在运行。真实材料和全周期资格未解除。

## 2026-09-25 15:18 UTC 增量：独立辐射与气体热源资格

[共同反应辐射单元](research/carbon-calcium-radiative-cell-v1/REPORT.md)完成25组高精度辐射来源、5组共同能量/熵率和两档30000s完整来源、元素/U/S及时间核查。辐射、气体与导热的能量和熵分别记账；原无辐射完整核查结果逐项相同。失败的结果序列化记录保留，物理预算未改。下一连续升降温程序已经事先冻结，真实材料与全周期资格不变。

## 2026-09-25 15:12 UTC 增量：跨库存域与完整相耗尽

[开放共同反应单元](research/carbon-calcium-domain-crossing-v1/REPORT.md)在原20000s全程核查通过后，延长至30000s，两档完整来源、元素/U/S、熵与时间预算通过。石墨、方解石先后耗尽，约99.26%初始碳外排；气相仍有碳且仍在交换，不是完整燃尽或稳态证明。[压力求根优化](research/carbon-calcium-rigid-flash-work-v1/REPORT.md)完整八格对照结果相同，实际耗时降低。共同反应32格全来源与时间资格已完成，空间温度/相量仍失败；64格两档正在计算。实际材料、有限速率、烧结及同材料全周期资格保持未通过。

## 2026-09-25 14:06 UTC 增量：变化外部气氛的共同反应单元资格

[3000s连续升降温与气氛程序](research/carbon-calcium-program-cell-v1/REPORT.md)完成独立高精度外界来源复算、两档全过程元素/U/S和时间核查，全部保持原预算。所录过程跨越四处相状态变化，包括石墨相耗尽；仅报告每秒观测括区，不冒充已独立验证的相事件时刻。虚拟输运、瞬时平衡与原受限元素域仍明确保留。原恒定气氛完整复核结果逐项相同。

## 2026-09-25 13:56 UTC 增量：共同反应单元开放气氛资格

[外部CO/CO2/O2/N2气体库](research/carbon-calcium-open-cell-v1/REPORT.md)的27组静态来源/熵关系及两档1000s全过程来源、元素/U/S积分、时间比较通过原预算。热与气体交换系数仍虚拟，局部反应仍瞬时平衡，不能称实砖燃烧速率。新的[有限速率论文单位审查](research/calcite-fernandez2019-unit-v1/REPORT.md)发现可解释的质量/摩尔归一化问题，但尚不能来源确认，未接入模型。

低温128更细轨迹完成且时间比較通过，完整局部能量审计仍在运行；表面128二阶结果局部U1.092e-6J超1e-6J，四阶待完成。开放64精细完成、两个空间事件通过，其他空间指标仍失败，完整审计继续。共同反应32格仍在计算。

## 2026-09-25 13:36 UTC 增量：共同反应十六格时间资格完成

[温度坐标十六格](research/carbon-calcium-temperature-column-v1/REPORT.md)两档全来源、逐格收支/熵、2/4求积及时间比较全部通过。8→16空间平均压力差99.589Pa刚低于100Pa预算，其余温度和相量三项仍失败；不称空间已收敛。继续事先冻结参数的三十二格阶段。真实材料、有限速率与全周期限制保持。

## 2026-09-25 13:12 UTC 增量：八格导数优化完整资格通过

[局部数值导数与解析熵行](research/carbon-calcium-temperature-column-v1/REPORT.md)已完成两档八格全来源/局部收支/熵、时间与旧路径对照，全部保持原预算。当前轨迹实测100–115s，旧路径114–130s；没有把局部计算提速比例冒充整体收益。已启动十六格继续空间核查。实材、有限速率、收缩、低高温同材料能量衔接和全周期预测仍未获资格。

## 2026-09-25 13:04 UTC 增量：共同反应八格时间资格完成

[温度坐标八格](research/carbon-calcium-temperature-column-v1/REPORT.md)的两档全来源、逐格收支/熵、时间预算全部通过。4→8空间四项仍超目标，平均压力差未单调减小，不能称空间已收敛。局部数值导数与解析熵行的逐点核查通过，减少全局账本导致的重复物性计算；同一八格条件下的完整优化路径对照正在进行，之后才推进更细网格。开放64格基础失败证据已推送，精细档及低温128格任务继续独立核查。

## 2026-09-25 12:53 UTC 增量：直接温度积分通过两格与四格资格

[直接温度坐标](research/carbon-calcium-temperature-column-v1/REPORT.md)保留两格首轮熵失败后，后两档全来源、逐格U/S和时间预算通过；四格也完成同样全审计及守恒坐标对照。四格实际39–46s，原路径273–296s，仅为这些算例的实测。空间误差保持原结论，已继续八格。开放64格基础在未接受Newton试算中氮指数下溢，最后接受时间13178.188823s，失败前缀归档中；不宣称真实氮量归零，精细档仍独立运行。实材与全周期限制未解除。

## 2026-09-25 12:47 UTC 增量：共同反应四格时间资格完成

[共同反应封闭列](research/carbon-calcium-column-v1/REPORT.md)四格两档全来源、逐格收支/熵、2/4求积与时间比較通过；2→4空间四项仍全部超预算，未授予连续体收敛结论。四格完整记录无损归档。下一数值路径用直接温度积分减少嵌套反演；其首轮基础档石墨相出现时一步熵超限，精细档通过，进一步收紧已开始。原失败保留，尚未启动该路径四格。

## 2026-09-25 12:28 UTC 增量：共同反应空间两格资格完成

[Ca/C/O闭列](research/carbon-calcium-column-v1/REPORT.md)两格解析Jacobian版本的全程来源/局部收支/熵、时间比较和原两单元对照全部通过；18组同相熵凹性、输运耗散结构和空间线性模态也通过。四格两档已开始，空间门槛事先冻结。该结果仍是理想气体、常固体体积、瞬时平衡及虚拟输运的条件数值资格。

## 2026-09-25 12:19 UTC 增量：开放32格全审计通过，继续64格

[开放连续程序](research/calcite-rigid-ramp-v1/REPORT.md)32格两档的全来源/逐格收支/熵/2-4求积和时间比较通过；16→32空间温度、压力、相量及事件仍全部超原预算，已按相同定义启动64格。原失败均保留，不用加密中的任务代替已验证结果。

共同反应的[单元库存与面导数](research/carbon-calcium-rigid-tangent-v1/REPORT.md)全部独立来源核查通过；新[空间装配](research/carbon-calcium-column-v1/REPORT.md)2/4/8格初始总量、守恒与方向导数核查通过，解析Jacobian两格轨迹完成、全程与旧算例对照审计在进行。真实材料与全周期资格不变。
