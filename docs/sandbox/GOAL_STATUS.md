...
下一步是[已独立推导的条件性压力传播接口](research/source-endpoint-comparison-v1/pressure-path.md)：用既有全域|uP|包络和稳定液相条件推导dP/dT界，绑定完整源域/库存/可用体积后再作温度误差传播。当前包络属制造条件假设，局部HEOS检查不是全域证书；推导尚未作为生产接口实现。随后按[既有根引导路线](research/source-prefix-trial-v1/next-route.md)获得实际重新求值的正库存接近区间。事件时间、运输成对液体/同供体焓、干界面/再润湿、来源保存/应用及完整原泥反应烧结冷却、三机制实测、全周期/多代仍未完成。
