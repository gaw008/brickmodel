...
当前执行约束以[合同第 0 节](../GOAL_BRICK_PHYSICS_SANDBOX.md#0-当前执行约束2026-09-12-用户调整)为准。代码基线 `08e2e8f`；本次仅调整合同与恢复入口，未修改物理代码、来源数据、验收矩阵或历史失败。Goal 实时读取为 `paused`，未启动 D1 或实验，也未修改面板目标文字或配置时间硬限额。
