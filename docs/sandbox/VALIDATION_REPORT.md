...
最新[来源库存首根与竞争](research/source-net-roots-v1/REPORT.md)：源码123项17.99s、最终非editable安装123项18.01s，117模块/124包文件匹配；17组提取前旧完整返回/失败记录不变。独立数学117组/1152断言、代码另588因式案例和Python29反例通过。安装回放原7试步14记录完成0.283783s，84库存全部严格正的no_roots，保留原拒步身份，无新EOS。这些验证纯数值插值的首根/排除与来源绑定，不能替代物理事件时刻、全区间真实轨迹或材料验证。下面保留各旧阶段的当时范围。
