...
2026-09-12[Arlabosse95°C](research/arlabosse95/README.md)完成离散来源接口：15读数/3unknown经原图像素与精确标尺独立复核；9点相对化学势以独立160项有理数atanh级数及严格尾界检验。原图重放、篡改/越界/缺失资产和当前/默认Decimal上下文负控完成；物性+旧Cp+CLI源码60项通过0.33s。没有新物理积分、插值或独立实验预测通过结论，安装验收另列。

2026-09-12[原生能量冷却宿主](research/cooling-energy-host-v1/REPORT.md)已完成V2两档0–10s实际运行：6046状态、6044接受步、42310RHS，粗细温差7.358699e−8K；最大局部/全局能量残差2.520e−13/4.899e−13J，原全部门通过。监督57.699928250s，进程回收、353输入未变。V1原0.3秒时间节点失败及61状态/60步独审前缀保持；新时钟只重排未提交区间，原精度/时刻/预算不变。源码及安装183项通过2.13/1.36s，另独立4项时钟解析通量/预算回归通过。资格限定制造半板原生E/功接口，真实材料/全周期未升级。

2026-09-12[热弹连续空间验证](research/cooling-spatial-v1/REPORT.md)已实际完成16/32/64格两档BDF，共六条0–10s轨迹；主细档L∞/L₂观察阶2.000423545/2.001065178、初始连续均温率阶1.996872478/1.999217908，原时间分离、空间/条件估计、账本、全插值域与资源门通过。监督合计7.126111124s，4269RHS/2058接受状态/606点，未重试或改参数。97原字节文件已归档；保存产物独审已复现606点/2058接受状态原门，最大局部/全局E2.922e−12/2.800e−11J。RHS只保存摘要/最坏输入，不能称全阶段原字段独审；各段存界与6个最坏多项式核查通过。参数仍制造分类，材料/全周期资格false。


2026-09-12[限定冷却热弹耦合](research/cooling-stress-v1/REPORT.md)完成实际安装两格绝热松弛与外冷却：原温度/局部全局能量/时间细化门槛通过，负对照漏耦合均检出；两条各原10s外部限额内完成，监督1.498250/1.493845s。相消缺陷与零流面问题已修，原RED/阈值保持；源码/安装各72项与独审10项通过。仍是常系数制造数值证据，该两格实验自身没有空间收敛证据；上述后续空间研究另列，同泥砖材料、相变或全周期资格仍未取得。
