...
最新[来源端点误差比较](research/source-endpoint-comparison-v1/REPORT.md)：源码81项63.75s、非editable安装81项63.19s，120模块/127源包文件一致；独审15被动反例及10测试含27精确代数案例通过。两版各30保存端点核算，安装0.202008s、无新EOS，全部结果除耗时外一致。完整原事件政策分别约束N/U/T/报告T处P；保存原生记录未声明来源事件政策，保持未判定。40原始归档成员重开逐字一致；完整压力传播仅有条件推导，事件时间、真实原泥材料和全周期等仍未完成。
