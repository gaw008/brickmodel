...
当前[来源完整单观测记录](research/source-observation-record-v1/REPORT.md)对应§9保存/查询的一部分：四种速率schema、严格源/库存/共享面关联、CLI/Python可用，源码/安装各93项及111原保存观测通过。整段运行/事件/失败账本重放、live resume和界面尚未由此完成；不满足§11完整模型条件。湿压力预算分析仍保留原数值事件失败和材料false。原Goal全部必需项不缩减。
