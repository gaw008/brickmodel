...
require({k:v['sha256'] for k,v in metadata['inputs_before'].items()}==supervision['inputs_after'] and supervision['inputs_unchanged'] is True,'all supervised before/after inputs equal')
...
for path,digest in supervision['inputs_after'].items():
    ...
    require(sha(raw)==digest and len(raw)==metadata['inputs_before'][path]['bytes'],'current supervised input byte match')
require(metadata['inputs_before'][str(Path(json.loads((RUN/'request.json').read_bytes())['case_path']))]['sha256']==raw_sha,'raw case bound to supervision')
...
saved_checkpoint=json.loads((RUN/'ordinary-checkpoint.json').read_bytes())
canonical=lambda v:json.dumps(v,sort_keys=True,separators=(',',':'),allow_nan=False).encode()
require(canonical(saved_checkpoint['checkpoint'])==canonical(checkpoint_wire(prefix.execution.checkpoint)),
        'all serialized checkpoint fields equal actual logged pause checkpoint')
require(sha(canonical(saved_checkpoint['checkpoint']))==saved_checkpoint['payload_sha256'],
        'independent checkpoint payload hash')
require(saved_checkpoint['source_resume_authorized'] is False and
        prefix.execution.checkpoint.next_step_s==F(1,64) and prefix.execution.checkpoint.initial_probe_done is True,
        'checkpoint next step and passive authority')
shape_labels={'raw graph schema','finite saved float','canonical saved fraction',
              'closed raw reference','acyclic requested raw data'}
result=dict(status='passed_with_documented_metadata_label_defect',check_count=len(checks),
    semantic_assertions=sum(k not in shape_labels for k in checks),
    raw_data_shape_assertions=sum(k in shape_labels for k in checks),
