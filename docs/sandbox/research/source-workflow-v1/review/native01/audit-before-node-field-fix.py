...
def signature(value, *, skip_elapsed=False):
    ...
    if isinstance(value, Node):
        fields = value.values
        if skip_elapsed and value.kind == 'sludge_sandbox.exact_integration.ExactIntegrationResult':
            fields = {k:v for k,v in fields.items() if k != 'elapsed_seconds'}
        return ('node', value.kind, tuple((k,signature(v,skip_elapsed=skip_elapsed)) for k,v in sorted(fields.items())))
