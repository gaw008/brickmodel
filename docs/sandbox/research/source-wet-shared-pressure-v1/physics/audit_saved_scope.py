...
def physical(x):
 if isinstance(x,dict):return {k:physical(v) for k,v in x.items() if k!='elapsed_seconds'}
 if isinstance(x,list):return [physical(v) for v in x]
 return x
for a,b in zip(t['candidates'],ot['candidates']):
 for key in ('terminal','projection','dry_result'):
  if key in a:ck(physical(a[key])==physical(b[key]),'unchanged full candidate '+key)
