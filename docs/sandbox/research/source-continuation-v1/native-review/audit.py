...
for row in rows:
 ...
 for cell in cells:
  ...
  ck(cell['water_balance_residual_mol']==cell['inventory_residual_mol'][0]+cell['inventory_residual_mol'][3]+cell['event_water_storage_roundoff_mol'])
...
for name,digest in a['runtime_before']['modules'].items():
 ck(hashlib.sha256((Path('/Users/wanggaoying/Desktop/brickmodel-github/src/sludge_sandbox')/name).read_bytes()).hexdigest()==digest)
for row in rows:
 for cell in row['cell_balances']:
  nr=cell['inventory_residual_mol']; w=cell['water_balance_residual_mol']; el=dict(cell['fluid_element_residuals_mol'])
  ck(el=={'H':2*w,'O':w+2*nr[1],'N':2*nr[2]})
 for label,value in row['fluid_element_residuals_mol']:
  ck(sum(dict(z['fluid_element_residuals_mol'])[label] for z in row['cell_balances'])==value)
ck(r['times_s'][0]['seconds']==F(a['exact_start']['numerator'],a['exact_start']['denominator']))
ck(r['times_s'][-1]['seconds']==F(a['exact_end']['numerator'],a['exact_end']['denominator']))
