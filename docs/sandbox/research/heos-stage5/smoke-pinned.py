...
(case/'smoke-result-pinned.json').write_text(json.dumps({'state':asdict(s),'chemical':asdict(c),'descriptor':json.loads(w.implementation.canonical_json),'default_canonical':_canonical(old),'heos_canonical':_canonical(w)},indent=2)+'\n')
