...
began=time.monotonic();path=Path(__file__).with_name('EXTRACT.json');data=json.loads(path.read_text())
case=json.loads(Path('/private/tmp/brick-source-dynamic-continuation-v1/native01/parent/case.json').read_text())
def q(v):
...
result={'clock_gate':data['clock_gate'],'clock_upper_s':q(data['clock_distance_upper_s']).__float__(),'original_gate_pressure_pa':case['event_policy']['pressure_absolute_pa'],'original_other_gates_all_true':all(all(row[:3]) for row in data['endpoint_gates']),'selected_failed_cells':[[i for i,v in enumerate(row) if v is not True] for row in data['cell_selected_pressure_gates']],'rows':rows,'proposed_new_numerical_profile':{'pressure_policy.pressure_tolerance_pa':1e-7,'inverse_policy.energy_tolerance_j':1e-6,'inverse_policy.temperature_tolerance_k_unchanged':1e-6,'all_other_physical_envelope_event_and_resource_fields_unchanged':True},'proposal_scope':'Budget allocation using recorded support; not a proof of future root bracket or full run success','elapsed_seconds':time.monotonic()-began,'eos_calls':0,'full_study_decodes':0}
Path(__file__).with_name('DIAGNOSIS.json').write_text(json.dumps(result,indent=2)+'\n')
