...
7. 原完整累计 balance 输出与分项时间账：已测 active 读档/审核/重建/执行/发布
   成本、已运行会话内暂停 wall、离线等待诊断、本进程监督 wall。原 cp.elapsed
   和父记录已测时间保留原值；历史聚合值不能事后猜拆成细分项。保存必须有明确
   最终化成本记录和暂停边界，不能把数据发布工作误归为离线等待。未完成发布
   仍是 unfinished attempt，不得从较早 cutoff 减掉这笔实际成本后恢复。
...
以下口径遵循已经提交的
`docs/sandbox/research/source-continuation-v1/review/RECONSTRUCTION_BOUNDARY.md:47`：

> Offline waiting time is not a measured solver execution time; keep it distinct from cumulative active work and the new process's supervisory wall deadline.

`S` 是已保存本段原累计计费 debit，包括全部已测 active 工作，以及按旧同会话
语义已经计入的在线暂停 wall；保留分项，不能把在线暂停说成 active 求解工作。
原历史记录只有聚合值时保存为原聚合项，不能猜测折扣。`D` 是明确关闭/交接到
重开之间的离线等待，只作诊断。`A` 是本次进程自 admission 起已测新增 active
工作及依旧按同会话规则计入的在线暂停；两者也分开列项。`C` 是原 cp.elapsed。
使用 Fraction 加法及原 `_upper_float` 形成：

```text
ordinary admission credit = S + A_before_integrate - C   # 必须 >= 0
ordinary cumulative charged wall = S + A_total
source cumulative charged wall   = parent_elapsed + S + A_total
offline waiting diagnostic       = D                    # 不进入以上额度
current process supervisory wall = monotonic_now - process_admission_begin
```

原 ordinary maximum_wall/steps/rejections 与 source lifetime outer/callback
额度均保持；原已测 S 和 parent_elapsed 不清零。读档、passive replay、来源
重建和发布的实际成本都进入 A；积分器内部 admission 重演由原 core 自己计时，
不能重复加一次。另给当前进程监督 deadline 独立起点、原配置给定的明确上限，
它用来限制本进程长时间悬挂，不是新的 source lifetime 额度；两种限制均检查。

持久化生命周期显式增加 `suspended_offline` 边界：完成保存/最终化并关闭旧 live
Session 后，至新进程 admission 前属于离线等待。原 `advance` 同会话暂停仍按
现有 begin 计入在线 wall，不能通过仅导出一份文件而仍保持 Session 在线来消除
成本。成功 save-and-suspend 后旧 live Session 不再 advance，新进程使用上述
已保存 debit；隔日重开不会仅因离线日期差超过原 180 秒计算预算。

本地实时时钟戳可用于 D 的诊断；若时钟倒退或无法比较，就把离线时长标为
unavailable/clock_discontinuity，不凭它判求解器 budget 用尽，也不要求时钟认证。
当前进程实际成本用 monotonic 计量。此口径保留所有旧已测 debit，不改变原
同会话实现；它明确区分新增离线生命周期，不能冒称旧 API 已支持此分类。
...
3. 原 study+普通段已用 callback 额度接近上限，恢复后下一真实请求被原总 cap
   阻断；ordinary step/rejection cap、S+A 计费 wall 和 source outer 都不能清零。
...
7. 合法隔日恢复仅记录 D，不增加求解工作 debit；原 S 已耗尽或本次读取/审核/
   重建使 S+A 超限仍拒绝。在线 pause、离线 suspended 与当前进程监督 deadline
   分别实际测；本地实时时钟倒退只使 D 不可用，不清除旧 active 成本。禁止可随意
   传入的 elapsed=0 默认。文件边界沿用 regular-file、FIFO、symlink/path 和重复 key。

## 本次文字修订记录

原稿保存在 `SOURCE_RESUME_NEXT_FIRST.md`，SHA256
`5c6ff6c447dc31b8851d3d0c7e2d612f8b18821614c61a39965318f61fd1ccb9`。
原稿把同会话在线暂停外推为跨进程离线等待也计费，写成 S+D+A；这与上面已提交
第 47 行约定不符。现修正为 S+A，D 独立诊断，并明确新增 save-and-suspend
生命周期边界、保留原在线暂停与所有旧已测成本。没有改变源码、既有政策数值或
当前 native 运行。
