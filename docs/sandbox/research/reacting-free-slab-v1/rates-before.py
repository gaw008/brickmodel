...
def solve_free_slab_rates(skeletons,*,normal_stretches,tangential_stretch,
                         pore_pressures_pa,external_pressure_pa,solid_inventories_mol):
    ...
    if (type(skeletons) is not tuple or not skeletons or
            any(type(s) is not DiagonalSkeletonEnergy for s in skeletons)):
    ...
    eta=tuple(F(_number(s.viscosity_pa_s,'viscosity',positive=True)) for s in skeletons)
    ...
    return FreeSlabRates(rates,rate_errors,states,normal_values,normal_errors,
