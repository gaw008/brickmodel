"""One bounded HEOS/source-mass root approach with manufactured geometry.
...
OUTER_SECONDS = 210.
CALLBACK_CAP = 16
TOTAL_CALLBACK_CAP = 33  # One initial rate probe, at most two 16-callback trials.
HELPERS = {
...
def run(root, output):
    ...
    result = dict(status='started', material_qualified=False, event_admitted=False,
        scope='N1_closed_low_temperature_source_integration_with_manufactured_geometry_and_transport',
    ...
    try:
        ...
        require(len(result['captures'])==1+len(seed.captures)+out.new_evaluations, 'all_actual_attempts_retained')
