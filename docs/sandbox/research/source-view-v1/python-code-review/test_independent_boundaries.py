...
def test_malformed_saved_event_is_a_controlled_error(source_bundle, event):
    ...
    _seal(source_bundle)
    try:
        result=view.inspect_source_run(source_bundle, capture_index=0, cell_index=0)
    except view.RunError:
        return  # A controlled data error also satisfies the review contract.
    ...
    assert not result['result']['material_qualified']
