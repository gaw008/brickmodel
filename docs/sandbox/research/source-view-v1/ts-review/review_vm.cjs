...
(async () => {
  ...
  const observations = [];
  {
    ...
    observations.push({name: 'failed revalidation retains unmarked old verified summary', message: h.elements.get('message').textContent,
      ...
      oldDataRetained: h.run('sourceData.study.record_sha256 === "old-verified-record"'), clearedRows: h.elements.get('source-values').tbody.children.length});
    assert.equal(observations.at(-1).summaryStillVerified, true);
  }
  {
    const h = harness(); h.context.fixture = value(); h.run('renderSource(fixture)');
    ...
    observations.push({name: 'stale source request failure overwrites current notification', message: h.elements.get('message').textContent, selectedCaptureState: h.elements.get('source-capture-state').textContent});
    assert.equal(h.elements.get('message').textContent, 'OLD_REQUEST_ERROR');
  }
  ...
  fs.writeFileSync('/private/tmp/source-view-v1/ts-review/VM_RESULTS.json', JSON.stringify(output, null, 2) + '\n');
