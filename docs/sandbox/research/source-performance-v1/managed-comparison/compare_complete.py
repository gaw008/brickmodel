...
def main():
    ...
    manifest_paths = [REPO/'data/sandbox/water'/name for name in
        ('heos-8.0.0-approved-manifest.json', 'heos-8.0.0-rhs-v2-manifest.json')]
    case_changes = {path: (before, after) for path, before, after in differences(*cases)}
    assert case_changes == {
        '/profile': ('source_multicell_wet_to_dry_heos_v1',
                     'source_multicell_wet_to_dry_heos_rhs_v2'),
        '/assets/10/path': tuple(str(p.relative_to(REPO)) for p in manifest_paths),
        '/assets/10/bytes': tuple(len(p.read_bytes()) for p in manifest_paths),
        '/assets/10/sha256': tuple(sha(p.read_bytes()) for p in manifest_paths),
    }, case_changes
    descriptors = [n['53']['fields']['canonical_descriptor'] for n in nodes]
    old_desc, new_desc = map(json.loads, descriptors)
    manifests = [raw(p) for p in manifest_paths]
    ...
    adverse = []
    original_pressure = float.fromhex(nodes[1]['29']['fields']['pressure_pa']['binary64'])
    for name, node, field, value in (
        ('pressure_one_ulp', '29', 'pressure_pa',
         {'binary64': math.nextafter(original_pressure, math.inf).hex()}),
    ...
    result.update(status='PASS', graph_counts=counts[0], exact_graph_changes=changes,
        config_changes={p: {'before': a, 'after': b} for p, (a, b) in case_changes.items()},
