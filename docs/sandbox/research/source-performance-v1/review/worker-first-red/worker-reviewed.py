...
def _load_request(raw):
    ...
    try:
        ...
        _require(value['time']['numerator'] >= 0 and value['time']['denominator'] > 0,
...
def _execute_request(request_path):
    ...
    try:
        ...
        _publish(output / 'RESULT.json', dict(status='completed', execution_mode=request['execution_mode'],
            ...
            managed_audit=None if admission is None else _worker_audit(admission),
        ...
        try:
            ...
            if primary is None:
                raise
            primary.add_note('source RHS profile save also failed: ' + repr(saving))
