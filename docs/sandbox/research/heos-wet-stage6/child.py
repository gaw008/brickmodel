...
try:
    ...
    ref=FixedInventoryEntropyReference(inputs,rp,reference_water)
    ...
    wi=reference_water.state_tp(300.,ref.initial_pressure_pa,phase='liquid');wf=reference_water.state_tp(point.temperature_k,point.pressure_pa,phase='liquid')
    refs={'elastic':.5*1000*1.4e-4*(3*math.log(lam))**2,'interface':.0015*(lam**2-1),'pore':reference_water.reference.molar_mass_kg_mol*(wf.native_internal_energy_j_kg-wi.native_internal_energy_j_kg)+(.01*(30-inputs.gas_constant_j_mol_k)+10)*(point.temperature_k-300),'body':0.,'dissipation':0.}
