...
def _raw_value(graph, value):
    """Resolve bounded raw references once while retaining shared subgraphs."""
    memo, active, visited = {}, set(), 0

    def visit(item, depth):
        nonlocal visited
        visited += 1
        _require(depth <= 96 and visited <= MAX_RAW_VALUES, 'raw_work_limit')
        if type(item) is dict and set(item) == {'ref'}:
            key = item['ref']
            _require(type(key) is str and key not in active and key in graph['nodes'], 'raw_reference')
            if key not in memo:
                active.add(key)
                memo[key] = visit(graph['nodes'][key], depth + 1)
                active.remove(key)
            return memo[key]
        ...
        _require(item is None or type(item) in (str, int, bool, float), 'raw_scalar')
        return item

    return visit(value, 0)


def _raw_same(left, right):
    """Type-sensitive comparison, with a bound on distinct shared node pairs."""
    visited = set()
    ...
    def compare(a, b):
        ...
        if pair in visited:
            return True
        visited.add(pair)
        _require(len(visited) <= MAX_RAW_VALUES, 'raw_comparison_work_limit')
        if type(a) is dict:
...
def _owned_audit(value):
    ...
    def visit(item, depth):
        nonlocal visited
        visited += 1
        _require(depth <= 96 and visited <= MAX_RAW_VALUES, 'audit_work_limit')
        if isinstance(item, Mapping) or type(item) in (tuple, list):
            key = id(item)
            _require(key not in active, 'audit_cycle')
            if key not in memo:
                active.add(key)
                if isinstance(item, Mapping):
                    _require(all(type(name) is str for name in item), 'audit_string_keys')
                    result = {name: visit(child, depth + 1) for name, child in item.items()}
                else:
                    result = [visit(child, depth + 1) for child in item]
                memo[key] = result
                active.remove(key)
            return memo[key]
        _require(item is None or type(item) in (str, int, bool, float), 'audit_json_value')
        _require(type(item) is not float or math.isfinite(item), 'audit_finite_value')
        return item
...
def _event_input(graph, fields, state, when, capture, meta):
    _require(_raw_same(_raw_value(graph, fields['state']), _raw_expected(state)) and
             _raw_same(_raw_value(graph, fields['time']), _raw_expected(when)) and
    ...
    _require(live.get('live_reference_only') is True and
             _raw_same(live.get('identity'), _raw_expected(meta['operator_identity'])) and
             _raw_same(live.get('modes'), _raw_expected(meta['interface_modes'])), 'journal_callback_operator')
...
def _audit_observations(checkpoint, observations, entries, meta):
    ...
    for ordinal, (original, capture, validated) in enumerate(zip(
        ...
        if original.validated:
            ...
            _require(_raw_same(_raw_value(graph, fields['evaluation']),
...
def _audit_controller(checkpoint, last, entries, meta, step_sizes):
    ...
    for event in reconstructed:
        ...
        _require(all(_raw_same(fields.get(key), _raw_expected(value)) for key, value in expected.items()),
                 'original_reconstructed_problem')
        provenance = fields['adapter_provenance']['fields']
        _require(all(_raw_same(provenance[key], _raw_expected(meta[key])) for key in
                     ('operator_identity', 'fixed_dry_mass_kg')) and
                 _raw_same(provenance['energy_model_identity'], _raw_expected(meta['energy_identity'])),
    ...
    _require(event['event'] == 'ordinary_segment_returned' and
             _raw_same(_raw_value(event['payload'], event['payload']['root']), _raw_expected(last)),
...
def read_source_trajectory_checkpoint(directory: str | Path) -> SourceTrajectoryCheckpointRecord:
...
def save_source_trajectory(session: SourceTrajectorySession, output: str | Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            exc.add_note('source trajectory manifest revocation: ' + type(secondary).__name__)
        try:
...
def restore_source_trajectory(saved_directory: str | Path, output: str | Path, *,
                              ...
                              managed_execution: bool = False) -> SourceTrajectorySession:
    ...
    try:
        ...
        raise SourceTrajectoryRecordError('source_trajectory_record_restore_already_claimed_or_missing') from exc
