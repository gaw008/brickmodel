...
RESUME_PROFILE = 'source_multicell_wet_to_dry_heos_resume_v4'
...
def required_source_assets(profile):
    ...
    if profile in (RHS_PROFILE, WORKFLOW_PROFILE, RESUME_PROFILE):
        manifest = {RHS_PROFILE: RHS_MANIFEST_ASSET, WORKFLOW_PROFILE: WORKFLOW_MANIFEST_ASSET,
                    RESUME_PROFILE: RESUME_MANIFEST_ASSET}[profile]
...
def _validate(values: dict) -> None:
    ...
    _require(values['schema'] == SCHEMA and values['profile'] in (PROFILE, RHS_PROFILE, WORKFLOW_PROFILE, RESUME_PROFILE) and
...
class SourceRunAssets:
    ...
    def check(self) -> None:
        _require(type(self.root) is type(Path()) and self.root == self.root.resolve() and
                 ...
                 (REQUIRED_ASSETS, required_source_assets(RHS_PROFILE), required_source_assets(WORKFLOW_PROFILE), required_source_assets(RESUME_PROFILE)) and
