        ...
        for index, call in enumerate(rhs, 1):
            ...
            for item in call['checks']:
                check(item['status'] == 'verified' and item['fluid_sha256'] == manifest['fluid_sha256'] and
                      item['config_sha256'] == canonical_digest(manifest['config']), 'unchanged actual native fluid/config')
...
def audit_packet(journals, paused, checkpoint):
    ...
    actual_files = {path.relative_to(directory).as_posix() for path in directory.rglob('*')
                   if path.is_file() and '.restore-attempt' not in path.relative_to(directory).parts and path.name != 'manifest.json'}
...
def audit_supervision(outcomes):
    ...
    check(metadata['inputs_before'] == status['inputs_after'], 'supervised before/after input identity')
    for path, sha in status['inputs_after'].items():
        check(digest(path) == sha, 'supervised input still unchanged '+path)
...
def main():
    ...
    check(any(value != 0. for result in raw_returns['resumed'] for value in result['evaluation'].rates.face_energy_w.values[1:-1]), 'nonzero internal heat flux')
    deltas = []
