...
def _audit_observations(checkpoint, observations, entries, meta):
    ...
    _require(sorted(references) == sorted(actual), 'unmatched_source_callback_event')
...
def save_source_trajectory(session, output) -> dict:
...
def restore_source_trajectory(saved_directory, output, *, cancel=None, managed_execution=False):
