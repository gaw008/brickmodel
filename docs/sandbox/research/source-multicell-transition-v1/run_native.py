"""Prepared single N3 HEOS/source mixed wet-to-dry study; importing this file runs no EOS.
...
PREVIOUS_RUNNER = 'source-dry-shared-pressure-v1/run_native.py'
PREVIOUS_RUNNER_SHA256 = 'f0c8a659053630ccad55901b0a7f6bf16c07b35313f54ce596087f54f277196b'
HELPERS = {
...
def run(root, output):
    ...
    result = dict(status='started', numerical_comparison_completed=False,
        ...
        scope='explicit_N3_source_liquid_transport_single_selected_cell_transition',
        ...
        initial_liquid_mol=INITIAL_LIQUID_MOL, initial_gas_mol=INITIAL_GAS_MOL,
    ...
    try:
        ...
        with patch.object(HEOSCandidate, '__init__', counted_init):
            template_storage, _ = source.make_case(root)
        ...
        # Independent spatial volume parameters; all paths retain these actual
        ...
        def observed(self, packed, at):
            ...
            item = dict(ordinal=len(result['captures'])+1, phase=phase_name,
                ...
                same_declared_storage_object=self.column.storages[SELECTED_CELL] is shared.storage,
        ...
        with patch.object(ExactSourceColumn, 'evaluate', observed):
            ...
            phases = tuple(cell.phase.phase_water_mol_s for cell in first.source_evaluation.cells)
            ...
            require(all(_same(source_mass, value.water.reference.molar_mass_kg_mol) for value in storages),
