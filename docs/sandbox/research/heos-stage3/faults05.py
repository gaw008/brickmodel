...
try:
    ...
    path=case/'bad-manifest05.json';path.write_text(json.dumps(bad))
    failure('bad_adapter_manifest',lambda:HEOSCandidate(path,root/'data/sandbox/water'),WaterSourceError)
finally:
    (case/'fault-results05.json').write_text(json.dumps({'passed':len(results)==5,'cases':results},indent=2)+'\n')
