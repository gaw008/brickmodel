        ...
        with self._lock:
            try:
                ...
                for iteration in range(8):
                    ...
                    self.last_coexistence.append({'iteration':iteration,'rho':list(densities),'liquid':left,'vapor':right,'dp':fp,'dg':fg})
                    if iteration>0 and abs(fp)<=1e-4 and abs(fg)<=1e-6:
                        break
