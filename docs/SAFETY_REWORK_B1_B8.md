...
| B8 independent semantics | SHA-256/size remains byte integrity only. Forward contract 4.0 binds case/fidelity/seed/parameter/source/solver provenance and strict mode independently reruns the L0/L1 ODE/FVM solver. Replay primary temperature/extents/species inventories/volume/porosity/heat states are compared point-by-point before replay-derived pressure/liquid/stress/summary/extrema/conservation/CSV; algebraic self-consistency and endpoint enthalpy identity cannot substitute. Comparisons canonicalize mapping keys and use 5× solver rtol/atol. Explicit custom fixtures are `not_evaluated`/integrity-only. Tiny/default inverse strict mode continues to reconstruct budget/seed/versioned Sobol designs and rerun the complete L0/policy/L1-shortlist/Pareto pipeline. | `io/artifacts.py`, `models/common.py`, `inverse/search.py` | coherent rehashed +10 K temperature and molar-inventory attacks; machine-roundoff/mapping-order controls; missing/tampered provenance/solver digests; original L0/L1 plus inverse replay; prior schema/summary/Pareto matrix |
