"""Record a source-defined reactive cell and continuous gas reservoir program."""
import argparse
import json
...
def main():
    ...
    atol = np.array([policy['inventory_absolute_tolerance_mol']] * 3
        ...
        + [policy['entropy_ledger_absolute_tolerance_j_k']] * 2) * factor
    ...
    with args.output.open('x') as stream:
        ...
        def record(kind, at, values):
            state, reservoir, contact = cell.observe(at, values)
            return {'kind': kind, 'time_s': float(at), 'values': values.tolist(),
                    'state': state, 'reservoir': reservoir, 'face': contact}

        emit({'kind': 'input', 'settings': p, 'face_parameters': face, 'rigid_parameters': rigid,
              'pressure_parameters': pressure, 'sources': sources, 'boundary_program': p['boundary_program'],
        ...
        knots = p['boundary_program']['knot_times_s']
        for segment, (start, end) in enumerate(zip(knots[:-1], knots[1:], strict=True)):
            solver = BDF(rates, start, values, end,
