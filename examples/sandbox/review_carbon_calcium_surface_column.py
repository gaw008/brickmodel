"""Source and local/global rate qualification of the zero-storage surface column."""
import argparse
import json
import math
...
def main():
    ...
    for count in policy['cell_counts']:
        column = CarbonCalciumSurfaceColumn(model, p, cell_p, face, rigid['numerics'], count, surface_p)
        ...
        for at in policy['times_s']:
            ...
            states, faces, _, interface = column.observe(at, values)
            contact, rad = interface['inner'], interface['radiation']
            ...
            reference_surface = solve_float(source,refs[-1],bath,column.surface_interior,face,
                dict(cell_p['radiation'],reservoir_temperature_k=rad_t),surface_p)
            exterior, interior = reference_surface['outer'], reference_surface['inner']
            radiation_flux = reference_surface['radiation']
            ...
            reference[-1, :4] += np.array([*interior['inventory'], interior['energy']])
            reference[-1, 4] += interior['entropy'][1]
            productions.extend([exterior['dissipation'],interior['dissipation'],radiation_flux['entropy_production_w_k']])
            ...
            result = {'cell_count': count, 'time_s': at, 'values': values.tolist(), 'states': states,
                'faces': faces, 'surface_exchange': interface, 'source_surface': reference_surface, 'radiation': rad, 'rates': actual.tolist(),
