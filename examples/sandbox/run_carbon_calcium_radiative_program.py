"""Record a reactive capsule and separately programmed gas/radiation sources."""
import argparse
import json
    ...
    with args.output.open('x') as stream:
        ...
        def record(kind, at, values):
            ...
            return {'kind': kind, 'time_s': float(at), 'values': values.tolist(),
                    'state': state, 'reservoir': reservoir, 'face': contact, 'radiation': cell.radiation(at, state)}
