"""Record a positive-inventory reactive column with programmed gas/radiation."""
import argparse
import json
...
def main():
    ...
    with args.output.open('x') as stream:
        ...
        def record(kind, at, values):
            states, faces, reservoir, contact, radiation = cell.observe(at, values)
            return {'kind': kind, 'time_s': float(at), 'values': values.tolist(),
                    'states': states, 'faces': faces, 'reservoir': reservoir, 'contact': contact, 'radiation': radiation}
