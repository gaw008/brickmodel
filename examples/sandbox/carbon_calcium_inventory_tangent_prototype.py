"""Experimental scaled-element derivatives for trace-gas coexistence.
...
def rigid_tangent(model,state,volume_m3,calcium_atoms_mol,carbon_atoms_mol,oxygen_atoms_mol):
    ...
    if present:
        ...
        if coexist:
            difference=math.fsum((ot,-ca,-ct,-ct))
            scale=n['CO']+2*n['O2']+abs(difference)
            oxygen_row=[-n['CO']/scale,0.,2*n['O2']/scale,0.,0.]
            oxygen_rhs=[0.,-2/scale,1/scale,0.]
        else:
            oxygen_rhs=[0.,0.,1/ot,0.]
        matrix=[carbon_row,oxygen_row,reaction]
        rhs=[[0.,1/ct,0.,0.],oxygen_rhs,[(entropy['CO2']-entropy['CO']-entropy['O2']/2)/rt,0.,0.,-1/(2*ng)]]
    if coexist:
    ...
    gas_sum_derivative=sum(dn[k] for k in gas_names)/ng
    logarithmic_gas={name:logarithmic[i] for i,name in enumerate(names)}
    logarithmic_gas['N2']=dn['N2']/n['N2']
    dmu={k:-entropy[k]*np.array([1.,0.,0.,0.])+rt*(logarithmic[-1]+logarithmic_gas[k]-gas_sum_derivative) for k in gas_names}
