"""Plot the recorded rigid-mixture cycle without running a solver."""
import argparse
import json
...
def main():
    ...
    ax=axes[0,1]
    for key,label in [('lime_mol','CaO formed'),('co2_mol','Retained CO2')]:ax.plot(time,series(key)*settings['amount_micromoles_per_mol'],label=label)
    ax.set_ylabel('Amount (μmol)');ax.legend(fontsize=8)
    ax=axes[0,2];ax.plot(time,series('pressure_pa')/settings['pressure_pa_per_bar'],label='Total pressure');ax.plot(time,series('co2_partial_pressure_pa')/settings['pressure_pa_per_bar'],label='CO2 partial pressure')
    ax.set_ylabel('Pressure (bar)');ax.legend(fontsize=8)
    ax=axes[1,0];kj=header['affinity_parameters']['joules_per_kilojoule']
    ax.plot(time,values[:,1]/kj,label='External heat in');ax.plot(time,(values[:,0]-values[0,0])/kj,ls='--',label='Total internal energy change')
    ...
    ax=axes[1,2];ax.plot(time,series('gas_volume_m3')*settings['gas_volume_cubic_centimetres_per_m3'],label='Gas occupied volume only')
    ax.set_ylabel('Gas volume (cm³)');ax.legend(fontsize=8)
