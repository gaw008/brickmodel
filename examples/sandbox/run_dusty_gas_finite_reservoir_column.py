"""Record finite reservoirs and pore storage under isothermal Darcy-Knudsen transport."""
import argparse
...
def main():
    ...
    column = FiniteReservoirDustyGasColumn(p, count, gas_constant, masses, viscosities, products)
    ...
    with args.output.open('x') as stream:
        ...
        def record(kind, at, values):
            ...
            inventories = column.volumes[:,None]*concentrations
        ...
        emit({'kind': 'input', 'settings': p, 'local_law_settings': local_p,
            ...
            'storage_order': 'left reservoir, pore cells from left to right, right reservoir',
            'storage_volumes_m3': column.volumes.tolist(),
