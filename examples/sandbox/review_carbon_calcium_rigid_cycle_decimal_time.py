"""Independent rigid calorimetric clock through elemental and volume equations."""
import argparse
import json
...
from carbon_calcium_rigid_decimal_reference import caloric_capacity
...
def main():
    ...
    header=rows[0];model,_,_=build(root,header['pressure_parameters']);inventory=header['inventory'];p=header['settings'];rigid=header['rigid_parameters'];volume=rigid['virtual_volume_m3']
        ...
        for segment,start in starts.items():
            ...
            for event in sorted((row for row in events if row['segment_index']==segment),key=lambda row:row['time_s']):
                ...
                for node,weight in zip(nodes,weights,strict=True):
                    t=(a+target)/2+(target-a)*node/2;candidate=model.at_temperature_volume(float(t),volume,*inputs,rigid['numerics'])
                    reference=caloric_capacity(model,t,volume,inventory,candidate,settings);cp=reference['cv'];minimum_cp=min(minimum_cp,cp)
                    max_cp=max(max_cp,abs(mp.mpf(candidate['equilibrium_cv_j_k'])-cp));total+=weight*(target-a)/2*cp/(g*(wall-t))
