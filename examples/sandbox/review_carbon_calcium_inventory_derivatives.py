    ...
    for inventory in settings['inventories']:
        ...
        for pressure in settings['pressure_points_pa']:
            for temperature in settings['temperature_points_k']:
                candidate = at_temperature_pressure(model,temperature,pressure,inventory)
                candidate.update(caloric_response(model,candidate))
                phases = (candidate['calcium_phase'],candidate['carbon_phase'])
                print(json.dumps({'started_inventory':inventory['name'],'temperature_k':temperature,'pressure_pa':pressure}),flush=True)
                for step_pair in settings['difference_steps']:
                    ...
                    for tt,pp in [(t-dt,p),(t+dt,p),(t,p-dp),(t,p+dp)]:
                        runtime = at_temperature_pressure(model,float(tt),float(pp),inventory)
