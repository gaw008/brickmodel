"""Independent element and chemical-potential equations for trace coexistence.

The oxygen-minus-two-carbon row is normalized by its own positive term sum.
This preserves the original elemental equation while resolving a nearly exact
CaCO3 stock whose CO and O2 amounts can be below 1e-40 mol. No gas-pressure
formula or coexistence constant from the candidate is used in the reference.
"""
    ...
    def equations(*coordinates):
        ...
        if carbon_phase == 'graphite_present':
            ...
            if calcium_phase == 'coexistence':
                difference = ot - ca - 2*ct
                oxygen_row = (2*n['O2'] - n['CO'] - difference) / (2*n['O2'] + n['CO'] + abs(difference))
            else:
                oxygen_row = oxygen
            result = [(n['calcite'] + n['CO'] + n['CO2']) / ct - 1, oxygen_row,
