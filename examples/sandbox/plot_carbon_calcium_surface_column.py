"""Plot recorded surface and bulk states with independent reservoir accounting."""
import argparse
import json
...
def main():
    ...
    axes[0, 0].plot(times, [r['surface_exchange']['surface']['temperature_k'] for r in rows], '-.', label='Zero-storage surface')
    ...
    axes[1, 0].plot(times, [r['surface_exchange']['outer']['energy_flow_w'] for r in rows], label='Gas and conduction')
    axes[1, 0].plot(times, [r['radiation']['energy_in_w'] for r in rows], label='Radiation')
    axes[1, 0].plot(times, [r['surface_exchange']['inner']['energy_flow_w'] for r in rows], '--', label='Inner face to body')
