"""Full source, surface, local and exterior integrals of a zero-storage-surface column."""
import argparse
import json
import math
...
def audit(path, root):
    ...
    surface_maxima = {k: 0. for k in ('temperature_k','pressure_pa','chemical_potential_j_mol','species_mol_s','energy_w','entropy_w_k')}
    surface_temperature_range = [float('inf'),-float('inf')]
    ...
    def review(states, values, at, category, row=None):
        ...
        surface = solve_float(source,refs[-1],bath,header['surface_interior_parameters'],header['exterior_parameters'],
            dict(cp['radiation'],reservoir_temperature_k=rad_t),header['surface_parameters'])
        f, inner, rad = surface['outer'],surface['inner'],surface['radiation']
        recorded_surface = row['surface_exchange'] if row is not None else None
        face_review(f,recorded_surface['outer'] if row is not None else None,header['exterior_parameters'])
        face_review(inner,recorded_surface['inner'] if row is not None else None,header['surface_interior_parameters'])
        surface_temperature_range[0] = min(surface_temperature_range[0],surface['surface']['temperature_k'])
        surface_temperature_range[1] = max(surface_temperature_range[1],surface['surface']['temperature_k'])
        surface_maxima['species_mol_s'] = max(surface_maxima['species_mol_s'],
            *(abs(v) for v in surface['species_residuals_mol_s'].values()))
        surface_maxima['energy_w'] = max(surface_maxima['energy_w'],abs(surface['energy_residual_w']))
        surface_maxima['entropy_w_k'] = max(surface_maxima['entropy_w_k'],abs(surface['surface_entropy_w_k']))
        if recorded_surface is not None:
            actual = recorded_surface['surface']
            reference = surface['surface']
            for k in ('temperature_k','pressure_pa'):
                surface_maxima[k] = max(surface_maxima[k],abs(actual[k]-reference[k]))
            surface_maxima['chemical_potential_j_mol'] = max(surface_maxima['chemical_potential_j_mol'],
                *(abs(actual['chemical_potentials_j_mol'][k]-v) for k,v in reference['mu'].items()))
            surface_maxima['species_mol_s'] = max(surface_maxima['species_mol_s'],
                *(abs(v) for v in recorded_surface['species_balance_residuals_mol_s'].values()))
            surface_maxima['energy_w'] = max(surface_maxima['energy_w'],abs(recorded_surface['energy_balance_residual_w']))
            surface_maxima['entropy_w_k'] = max(surface_maxima['entropy_w_k'],abs(recorded_surface['surface_entropy_balance_w_k']))
        minima['production_w_k'] = min(minima['production_w_k'], rad['entropy_production_w_k'])
        incoming_energy = f['energy'] + rad['energy_in_w']
        body_rates[-1, :4] += np.array([*inner['inventory'], inner['energy']])
        body_rates[-1, 4] += inner['entropy'][1]
        production = math.fsum(productions + [f['dissipation'],inner['dissipation'],rad['entropy_production_w_k']])
    ...
    surface_flags = {k:v<=header['surface_parameters']['verification'][k] for k,v in surface_maxima.items()}
    surface_domain = header['surface_parameters']['surface_temperature_domain_k']
    surface_flags['source_domain'] = surface_temperature_range[0]>=surface_domain[0] and surface_temperature_range[1]<=surface_domain[1]
    flags.update(surface_balances_and_sources=all(surface_flags.values()),completed=terminal['kind'] == 'summary' and terminal['status'] == 'completed',
    ...
    return {'trajectory': str(path), 'cell_count': n, 'counts': counts, 'maxima': maxima, 'minima': minima,
        ...
        'surface_maxima':surface_maxima,'surface_within_budgets':surface_flags,'surface_temperature_range_k':surface_temperature_range,
