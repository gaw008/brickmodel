...
def main():
    ...
    result={'settings':settings,'routes':routes,'maximum_differences':errors,'within_budgets':flags,'all_requested_budgets_met':all(flags.values()),
        'profile_scope':'One state reconstruction per endpoint in both routes, followed by shared face rates and the caloric coordinate transform. Earlier duplicate-decoding profile is retained separately and cannot quantify this comparison.',
