"""Compare volume-matched composition observations across uniform meshes."""
import argparse
import json
...
def main():
    ...
    for coarse_name, fine_name in p['pairs']:
        ...
        for a, b in zip(coarse, fine, strict=True):
            x, y = np.asarray(a['mole_fractions']), np.asarray(b['mole_fractions'])
        ...
        comparisons.append({'coarse': coarse_name, 'fine': fine_name, 'observations': len(coarse),
            'same_times': same_times, 'maximum_parent_fraction_difference': maximum,
            'maximum_at_time_s': when, 'maximum_at_coarse_cell_and_species_zero_based': location,
            'maximum_mean_fraction_difference': mean_difference,
            'budget': p['maximum_parent_fraction_difference_budget'],
            'within_budget': same_times and maximum <= p['maximum_parent_fraction_difference_budget']})
