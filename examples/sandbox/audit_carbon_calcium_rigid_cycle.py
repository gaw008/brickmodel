"""Full rigid Ca/C/O temperature-coordinate heat, entropy and phase audit."""
import argparse
import json
import math
...
def audit(path,settings,root):
    ...
    model,_,_=build(root,header['pressure_parameters']);inventory=header['inventory'];rigid=header['rigid_parameters'];volume=rigid['virtual_volume_m3']
    inputs=[inventory[k] for k in ['calcium_atoms_mol','carbon_atoms_mol','oxygen_atoms_mol','nitrogen_molecules_mol']]
    u0=header['initial_internal_energy_j'];s0=initial['state']['entropy_j_k'];g=p['virtual_calorimeter']['heat_conductance_w_k']
    cvsource=sources['calcium_volume_source'];vsource=sources['solid_volume_source']
    volumes={name:float(row['volume_cm3_mol'])*cvsource['cubic_metres_per_cubic_centimetre'] for name,row in cvsource['phases'].items()}
    volumes['C']=float(vsource['graphite']['volume_cm3_mol'])*vsource['cubic_metres_per_cubic_centimetre']
    ...
    maxima={name:0. for name in ['element_mol','pressure_pa','reaction_gibbs_j_mol','source_energy_j','source_entropy_j_k','volume_m3','heat_ledger_j','entropy_ledger_j_k']}
    counts={'recorded':0,'dense':0};minimum_gas=minimum_solid=minimum_production=float('inf');events=[];transitions={}
    def source_review(state,category):
        nonlocal minimum_gas,minimum_solid
        counts[category]+=1;t=state['temperature_k'];pressure=state['pressure_pa'];n=state['amounts_mol'];gas={name:n[name] for name in ['CO','CO2','O2','N2']};ng=math.fsum(gas.values())
        ...
        for name,data in thermal.items():
            ...
            if name in gas:ss[name]-=r*math.log(pressure/p0*gas[name]/ng)
            else:hs[name]+=(pressure-p0)*volumes[name]
            mu[name]=hs[name]-t*ss[name]
        h=math.fsum(n[name]*hs[name] for name in n);s=math.fsum(n[name]*ss[name] for name in n)
        solid_v=math.fsum(n[name]*v for name,v in volumes.items());gas_v=ng*r*t/pressure;total_v=solid_v+gas_v
        u=math.fsum(n[name]*(hs[name]-pressure*volumes[name]) for name in volumes)+math.fsum(n[name]*(hs[name]-r*t) for name in gas)
        ...
        pressure_error=max(abs(pressure-ng*r*t/(volume-solid_v)),abs(math.fsum(state['partial_pressures_pa'].values())-pressure),
            *(abs(state['partial_pressures_pa'][name]-pressure*value/ng) for name,value in gas.items()))
        ...
        for name,value in [('element_mol',element_error),('pressure_pa',pressure_error),('reaction_gibbs_j_mol',max(carbon_error,calcium_error)),
            ('source_energy_j',max(abs(state['enthalpy_j']-h),abs(state['internal_energy_j']-u))),('source_entropy_j_k',abs(state['entropy_j_k']-s)),('volume_m3',max(abs(total_v-volume),abs(state['gas_volume_m3']-gas_v),abs(state['solid_volume_m3']-solid_v)))]:
            maxima[name]=max(maxima[name],value)
    for row in rows:
        if 'state' not in row:continue
        source_review(row['state'],'recorded')
        maxima['heat_ledger_j']=max(maxima['heat_ledger_j'],abs(row['state']['internal_energy_j']-u0-row['values'][1]))
    ...
    for order in budget['quadrature_orders']:
        ...
        for row in rows:
            ...
            dense=row['dense_output'];left,right=dense['start_time_s'],dense['end_time_s'];wall=p['virtual_calorimeter']['program'][row['segment_index']]['reservoir_temperature_k'];integral=np.zeros(3)
            for node,weight in zip(nodes,weights,strict=True):
                at=(left+right)/2+(right-left)*node/2;values=polynomial(row,at)
                state=model.at_temperature_volume(float(values[0]),volume,*inputs,rigid['numerics']);source_review(state,'dense')
            ...
            max_local=max(max_local,abs(state['internal_energy_j']-previous['state']['internal_energy_j']-integral[0]));max_global=max(max_global,abs(state['internal_energy_j']-u0-total[0]))
    ...
    for segment,start in transitions.items():
        ...
        for event in sorted((row for row in events if row['segment_index']==segment),key=lambda row:row['time_s']):
            ...
            duration,error=quad(lambda t:model.at_temperature_volume(t,volume,*inputs,rigid['numerics'])['equilibrium_cv_j_k']/(g*(wall-t)),last_temperature,tt,
            ...
            difference=event['time_s']-clock;temperature_error=event['state']['temperature_k']-tt;pressure_error=event['state']['pressure_pa']-float(boundary['reference_pressure_pa']);endpoint_error=abs(event['state']['amounts_mol'][disappearing])
            event_reviews.append({'segment':segment,'name':event['name'],'direction':event['direction'],'recorded_time_s':event['time_s'],
                ...
                'phase_pressure_error_pa':pressure_error,'phase_endpoint_error_mol':endpoint_error,'within_budget':abs(pressure_error)<=budget['phase_event_pressure_pa'] and abs(difference)<=budget['phase_event_time_s'] and abs(temperature_error)<=budget['phase_event_temperature_k'] and endpoint_error<=budget['phase_endpoint_mol']})
    ...
    flags={name:maxima[name]<=budget[name] for name in ['element_mol','pressure_pa','reaction_gibbs_j_mol','source_energy_j','source_entropy_j_k','volume_m3']}
    flags.update({'completed':rows[-1]['kind']=='summary' and rows[-1]['status']=='completed','positive_gas':minimum_gas>0,'nonnegative_solid':minimum_solid>=0,
        'heat_ledger':maxima['heat_ledger_j']<=budget['energy_j'],
...
def main():
    ...
    pressure=max(abs(left[t]['state']['pressure_pa']-right[t]['state']['pressure_pa']) for t in left)
    comparison={'samples':len(left),'maximum_temperature_difference_k':temperature,'maximum_pressure_difference_pa':pressure,'maximum_species_differences_mol':species,
        'within_budgets':pressure<=budget['time_pressure_pa'] and temperature<=budget['time_temperature_k'] and max(species.values())<=budget['time_species_mol']}
