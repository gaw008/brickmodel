...
assert r['material_qualified'] is False, 'material qualification must remain false'
...
actual = r['actual_run']
assert type(actual['feed_mass_g']) is str and type(actual['initial_moisture_percent']) is str, 'printed decimal strings required'
feed_mass = F(actual['feed_mass_g'])
moisture = F(actual['initial_moisture_percent'])
assert feed_mass > 0, 'positive feed mass required'
assert 0 <= moisture <= 100, 'moisture mass percent outside [0, 100]'
run_dry_mass = feed_mass*(100-moisture)/100
