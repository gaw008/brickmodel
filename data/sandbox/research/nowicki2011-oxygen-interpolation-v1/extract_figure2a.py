...
def main():
    ...
    svg = args.scratch / "page4-orig.svg"
    # Default SVG export shrinks/centers content while retaining the page size.
    # These options keep its units aligned with the original PDF and raster.
    command = ["pdftocairo", "-origpagesizes", "-nocenter", "-noshrink",
               "-f", "4", "-l", "4", "-svg", str(pdf), str(svg)]
    ...
    if len(stroked) != 1608 or len(selected["d"]) != 8695 or selected["stroke-width"] != "0.597":
        raise ValueError("verified solid curve path identity mismatch")
    if selected["transform"] != "matrix(1, 0, 0, -1, 254.62, 149.644)":
    ...
    vectors = {
        ...
        "solid_500C_main_path_clip": next(
            clip.find(SVG_NS + "path").attrib["d"]
            for clip in root.iter(SVG_NS + "clipPath") if clip.attrib["id"] == "clip-25"),
    ...
    facts = {
        ...
        "method": {"script": str(Path(__file__).resolve().relative_to(args.repo.resolve())),
                   ...
                   "coordinate_check": "SVG export uses original page size, no centering or shrinking; curve origin matrix matches native PDF page object193, content stream152: (254.62,641.222) bottom-left pt, i.e. (254.62,149.644) top-left pt.",
