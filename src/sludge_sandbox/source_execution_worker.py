...
def _execute_request(request_path):
    ...
    try:
        ...
        secondary = []
        recorder = None if budget is None else budget.recorder
        returned_status = result.get('status') if type(result) is dict else getattr(result, 'status', None)
        returned_reason = result.get('reason') if type(result) is dict else getattr(result, 'reason', None)
        ...
        status = ('resource_limit' if (budget is not None and budget.stop_reason is not None) or
                  returned_status == 'resource_limit' or
                  (recorder is not None and recorder.stop_status == 'resource_limit') else
                  'cancelled' if isinstance(exc, KeyboardInterrupt) or
                  returned_status == 'cancelled' or
                  (budget is not None and budget.user_requested) or
                  (recorder is not None and recorder.stop_status == 'cancelled') else
                  'domain_exit' if returned_status == 'domain_exit' else 'failed')
        if configured:
            try:
                _clear_workflow_deadline()
        ...
        returned_counts = result.get('counts') if type(result) is dict else getattr(result, 'counts', None)
        actual = (dict(recorder.counts) if recorder is not None else
                  None if returned_counts is None else dict(returned_counts))
        failure = dict(schema='source_execution_failure_v1', status=status,
            ...
            operation_status=returned_status, operation_reason=returned_reason,
