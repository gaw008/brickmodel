...
class CoolingThermoelasticPlate:
    ...
    def evaluate(self, temperatures_k) -> CoolingPlateEvaluation:
        ...
        # Every matrix row sums to C. Thus m=qbar/C where qbar is the mean
        # weighted by 1/Ce, and Tdot_i=m+(q_i-qbar)/Ce_i. Scale positive weights
        # by min(Ce), and center q before averaging/subtracting it: the original
        # (q_i-c_i*m)/Ce_i loses even the one-cell common mode as Ce -> 0.
        c = self.stress_free_heat_capacity_j_m3_k
        q_scale = max(abs(value) for value in q)
        if q_scale == 0.:
            mean_rate, rates = 0., (0.,) * cells
        else:
            pivot = min(range(cells), key=ce.__getitem__)
            weights = tuple(ce[pivot] / dd for dd in ce)
            weight_sum = _sum(weights, "inverse_capacity_weight_sum")
            scaled_q = tuple(qq / q_scale for qq in q)
            differences = tuple(qq - scaled_q[pivot] for qq in scaled_q)
            centered_mean = _sum((ww * dq for ww, dq in zip(weights, differences)), "centered_forcing_sum") / weight_sum
            # A separately compensated uncentered sum retains a small common
            # forcing when positive and negative q cancel. The centered sum is
            # retained for the contrast subtraction; both express the same mean.
            qbar = q[pivot] if not any(differences) else _number(
                _sum((ww * qq for ww, qq in zip(weights, scaled_q)),
                     "scaled_mean_forcing") / weight_sum * q_scale, "mean_forcing")
            mean_rate = _number(qbar / c, "mean_temperature_rate")
            rates = tuple(_sum((mean_rate, _number(q_scale * (dq - centered_mean) / dd,
                "contrast_temperature_rate")), "temperature_rate") for dq, dd in zip(differences, ce))

            # One roundoff correction places the representable common mode in
            # the lowest-Ce cell. This is checked against *every* heat equation
            # below; it cannot authorize a correction that changes the forcing.
            represented_mean = _sum((rate / cells for rate in rates), "represented_mean_temperature_rate")
            corrected = list(rates)
            corrected[pivot] = _sum((rates[pivot], cells * (mean_rate - represented_mean)),
                                    "common_mode_corrected_temperature_rate")
            rates = tuple(corrected)

        # edot must describe the *returned* rate field. Huge contrast rates can
        # make a small common mode unrepresentable in binary64. Reject that
        # numerical domain rather than returning mutually incompatible stages.
        actual_mean_rate = _sum((rate / cells for rate in rates), "actual_mean_temperature_rate")
        roundoff_limit = 64. * math.ulp(1.)
        if q_scale:
            forcing_rate_scale = _number(q_scale / c, "forcing_rate_scale", positive=True)
            common_error = abs(actual_mean_rate - mean_rate) / forcing_rate_scale
            if not math.isfinite(common_error) or common_error > roundoff_limit:
                raise ThermoelasticPlateError(
                    f"unresolved_thermal_rate_common_mode: relative_error={common_error:.17g}, limit={roundoff_limit:.17g}")
            for dd, cc, td, qq in zip(ce, coupling, rates, q):
                residual = _sum((dd * td, cc * actual_mean_rate, -qq), "thermal_rate_residual")
                relative_error = abs(residual) / q_scale
                if relative_error > roundoff_limit:
                    raise ThermoelasticPlateError(
                        f"unresolved_thermal_rate_equation: relative_error={relative_error:.17g}, limit={roundoff_limit:.17g}")
        strain_rate = _number(self.linear_expansion_per_k * actual_mean_rate, "in_plane_strain_rate")
        ...
        for flux, left, right in zip(faces[1:-1], temperatures[:-1], temperatures[1:]):
            face_entropy.append(0. if flux == 0. else _number(flux * ((left - right) / left) / right, "internal_entropy_production", nonnegative=True))
        face_entropy.append(0. if outer == 0. else _number(outer * ((temperatures[-1] - self.outer_temperature_k) / temperatures[-1]) / self.outer_temperature_k, "boundary_entropy_production", nonnegative=True))
