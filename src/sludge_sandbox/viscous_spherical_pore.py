...
class ViscousSphericalPore:
    ...
    def mechanics(self, radius, gas_pressure):
        ...
        total_volume = self.matrix_volume+pore_volume
        fraction = pore_volume/total_volume
        outer = (3*total_volume/(4*math.pi))**(1/3)
        rate = (radius*(gas_pressure-self.outside_pressure)-2*self.gamma)/(4*self.eta*(1-fraction))
        constant = radius**2*rate
        volume_rate = 4*math.pi*constant
        dissipation = 16*math.pi*self.eta*radius*(1-fraction)*rate**2
        surface_energy_rate = 8*math.pi*self.gamma*radius*rate
        return {'radius_m': radius, 'outer_radius_m': outer, 'pore_volume_m3': pore_volume,
                'total_volume_m3': total_volume, 'porosity': fraction,
                'gas_pressure_pa': gas_pressure, 'laplace_pressure_pa': 2*self.gamma/radius,
                'radius_rate_m_s': rate, 'outer_radius_rate_m_s': constant/outer**2,
                'radial_velocity_constant_m3_s': constant,
                'matrix_pressure_pa': self.outside_pressure-4*self.eta*constant/outer**3,
                'pore_volume_rate_m3_s': volume_rate,
                'porosity_rate_s': self.matrix_volume*volume_rate/total_volume**2,
                'surface_energy_j': 4*math.pi*self.gamma*radius**2,
                'surface_energy_rate_w': surface_energy_rate,
                'external_work_in_w': -self.outside_pressure*volume_rate,
                'viscous_dissipation_w': dissipation,
                'entropy_production_w_k': dissipation/self.temperature}
