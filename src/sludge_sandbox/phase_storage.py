...
class LiquidWaterPhase:
    ...
    def __post_init__(self):
        if type(self.water) is not WaterProperties:raise PhaseStorageError('verified_water_provider_required')

    @property
    def metadata(self):
        r=self.water.reference
        return PhaseMetadata('H2O','liquid',r.molar_mass_kg_mol,'mol_of_declared_species',_REFERENCE,r.source_ids,'derived_from_evidence')
